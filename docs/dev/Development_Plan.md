...
>
> Rust 전투 개입의 다음 prerequisite 순서는 **I2b-S1a → I2b-S1b → I2b-S2 → I2b-S3**다. S1a는 occurrence target/observer-combatant carrier와 full static roster `CombatOpportunityContext.combatant_ids: Vec<String>`만 소유하고(`combat_opportunity.rs`, wave1 tests, repo 전체 mandatory context literal compile fix), checkpoint/save/lifecycle/transaction/content authoring은 금지한다. Vec는 duplicate ingress evidence를 보존하며 기존 `evaluate(instances, context)` signature를 넓히지 않고 local validation이 empty·blank·duplicate와 observer mapping nonmembership을 canonicalization 전에 reject한다. Evaluation/pause fingerprint는 검증된 stable-ID-sorted clone을 사용하므로 unique input order에는 독립적이고 membership change에는 민감하다. Actual request roster equality는 S1b의 `CombatRuntime::with_opportunities`/restore가 evaluation/restore acceptance 전에 검사하고, runtime constructors는 request participants에서 canonical sorted Vec를 직접 derive한다. Active/hp 판단도 S1b runtime snapshot owner다. S1b는 `combat_runtime.rs`와 focused tests만 소유해 이 roster gate, trigger-tick 시작 health/active snapshot, stable pause ID, selected-target exact-one/no-fallback preflight, checkpoint v2와 provenance-less paused v1 reject를 구현하며 `save.rs`, `lib.rs`, GameState/terminal/transaction/authoring schema는 금지한다. 각 slice는 merge와 독립 review 후 다음 slice로 진행하며 exact acceptance/stop 조건은 [combat contract handoff](../content/design_source/handoffs/combat_contract_handoff.md)의 “WP-I2b-S1 승인 정본”과 “별도 work package와 acceptance”를 따른다. `SaveEnvelope` v2는 I7b까지 금지한다.
>
