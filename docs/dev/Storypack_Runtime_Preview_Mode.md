...
Status: 결정 문서 + `wuxia_mumyeong_request_for_aid` runtime 구현 완료 + `wuxia_mumyeong_awakening` next runtime selected
...
17. `wuxia_mumyeong_awakening` — 다음 runtime 후보
   - `wuxia_mumyeong_followup_after_failed_aid` handoff에서 선택했으며, 아직 preview runtime YAML/generated bundle에는 추가하지 않았다.
