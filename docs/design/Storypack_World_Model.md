...
- Notion 사건 카드 DB 26개와 후일담 카드 DB 17개는 future/source coverage로 추적하되, 현재 runtime/machine-readable mirror에 자동으로 모두 반영하지 않는다. Repo mirror는 runtime 승격된 final-entry 카드까지 포함해 `wuxia_jianghu_pack` 32개 후보 카드를 둔다.
...
- `docs/content/storypack_db/encounter_situations.json`에 `isolation_pack` 6개, `yageunmong_pack` 12개, `wuxia_jianghu_pack` 32개, 총 50개 repo 후보 카드를 둔다. Notion 사건 카드 DB의 26개 row는 `docs/dev/Notion_Design_Coverage.md`에서 매핑한 upstream source이며, machine-readable mirror에 전부 자동 import하지 않는다.
...
진행 상태: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt`, `wuxia_black_heaven_escape_price`, `wuxia_heavenly_archive_previous_outsiders`, `wuxia_wounded_shelter_dawn_offers`, `wuxia_mumyeong_first_sighting`, `wuxia_mumyeong_first_confrontation`, `wuxia_mumyeong_copy_style_reveal`, `wuxia_mumyeong_reads_orthodox_style`, `wuxia_mumyeong_midgame_reunion`, `wuxia_boss_first_appearance`, `wuxia_mumyeong_request_for_aid`, `wuxia_mumyeong_awakening`, `wuxia_qingliu_attack_after_war`, `wuxia_mumyeong_destroys_orthodox_sect`, `wuxia_boss_recruits_mumyeong`, `wuxia_mumyeong_departure_truth_summary`, `wuxia_seoharin_empty_place`, `wuxia_seoharin_left_meal`, `wuxia_sado_final_phase_1_price_tag`, `wuxia_sado_final_phase_2_weakpoint_control`, `wuxia_sado_final_phase_3_outside_calculation`, `wuxia_boss_resolution`, `wuxia_mumyeong_resolution`, `wuxia_seoharin_qingliu_resolution`, `wuxia_cheongirok_resolution`, `wuxia_black_serpent_aftermath`, `wuxia_final_epilogue_renderer_contract` runtime은 완료했다. 다음 후보는 final epilogue UX/playtest 및 contract follow-up 검증이다.

- 서른네 개 implemented preview encounter는 같은 `storypack_preview` bundle에만 둔다. `wuxia_black_serpent_aftermath`까지 YAML/generated bundle에 반영됐고, `wuxia_final_epilogue_renderer_contract`는 preview ending + Rust GameCore-owned structured body block consumer로 반영됐다.
- `wuxia_boss_resolution` handoff도 2026-06-02에 구현 완료된 과거 handoff이며, 보스 결산은 route seed bridge로만 남긴다.
- `wuxia_mumyeong_resolution`, `wuxia_seoharin_qingliu_resolution`, `wuxia_cheongirok_resolution`, `wuxia_black_serpent_aftermath`, `wuxia_final_epilogue_renderer_contract` handoff는 2026-06-02에 구현 완료된 과거 handoff이며, 최신 다음 후보는 final epilogue UX/playtest 및 contract follow-up 검증이다.
- 주요 handoff history는 `route_midgame_continuity_after_wounded_shelter`, `wuxia_mumyeong_first_confrontation_after_sighting`, `wuxia_mumyeong_followup_after_copy_style_reveal`, `wuxia_mumyeong_followup_after_orthodox_style_trace`, `wuxia_mumyeong_followup_after_midgame_reunion`, `wuxia_boss_followup_after_first_appearance`, `wuxia_mumyeong_followup_after_failed_aid`, `wuxia_mumyeong_followup_after_awakening`, `wuxia_qingliu_attack_after_war_followup`, `wuxia_mumyeong_destroys_orthodox_sect_followup`, `wuxia_boss_recruits_mumyeong_followup`, `wuxia_mumyeong_departure_truth_summary_followup`, `wuxia_seoharin_empty_place_followup`, `wuxia_seoharin_left_meal_followup`, `wuxia_sado_final_phase_1_price_tag_handoff`, `wuxia_sado_final_phase_2_weakpoint_control_handoff`, `wuxia_sado_final_phase_3_outside_calculation_handoff`, `wuxia_boss_resolution_handoff`, `wuxia_mumyeong_resolution_handoff`, `wuxia_seoharin_qingliu_resolution_handoff`, `wuxia_cheongirok_resolution_handoff`, `wuxia_black_serpent_aftermath_handoff`다.
- `wuxia_qingliu_attack_after_war_followup` docs-only handoff는 현악문 consequence trace를 다음 구현 대상으로 결정했고, 해당 runtime slice는 완료됐다.
...
- `wuxia_boss_first_appearance`는 preview runtime 구현 완료이며, 안정 choice id는 `read_the_boss_flow_and_fail_to_move`, `pull_seo_harin_behind_broken_gate`, `watch_mumyeong_answer_the_boss`, `retreat_before_the_second_step`다.
...
- 상태: 설계 문서화 + machine-readable storypack DB 검증 + preview mode 결정 + 무협 서른네 개 preview encounter runtime + final epilogue seed consumer + Web/terminal default 이구학지 wiring 완료.
...
- 런타임 구현: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt`, `wuxia_black_heaven_escape_price`, `wuxia_heavenly_archive_previous_outsiders`, `wuxia_wounded_shelter_dawn_offers`, `wuxia_mumyeong_first_sighting`, `wuxia_mumyeong_first_confrontation`, `wuxia_mumyeong_copy_style_reveal`, `wuxia_mumyeong_reads_orthodox_style`, `wuxia_mumyeong_midgame_reunion`, `wuxia_boss_first_appearance`, `wuxia_mumyeong_request_for_aid`, `wuxia_mumyeong_awakening`, `wuxia_qingliu_attack_after_war`, `wuxia_mumyeong_destroys_orthodox_sect`, `wuxia_boss_recruits_mumyeong`, `wuxia_mumyeong_departure_truth_summary`, `wuxia_seoharin_empty_place`, `wuxia_seoharin_left_meal`, `wuxia_sado_final_phase_1_price_tag`, `wuxia_sado_final_phase_2_weakpoint_control`, `wuxia_sado_final_phase_3_outside_calculation`, `wuxia_boss_resolution`, `wuxia_mumyeong_resolution`, `wuxia_seoharin_qingliu_resolution`, `wuxia_cheongirok_resolution`, `wuxia_black_serpent_aftermath`, `wuxia_final_epilogue_renderer_contract`는 완료했다. 결산 계열은 route와 후속 epilogue candidate seed를 남기는 bridge였고, final epilogue contract는 그 seed를 structured body block으로 소비한다. `yageunmong_pack` runtime은 미착수다.
- 다음 추천: `wuxia_final_epilogue_renderer_contract` UX/playtest 및 contract follow-up 검증. `docs/design/Wuxia_Final_State_Routing.md`의 final-state contract와 흑사방 aftermath seed를 기준으로 구현된 후일담 body block 표시를 검증하되, 서하린 truth delivery, `told_seoharin_truth`, return/settlement, Cheonggi Record identity reveal, legacy office bundle/legacy `escape-office` key/seed 기반 random copy-style system/천외편린 3택 성장/faction route graph/debt ledger/combat schema/full boss combat은 바로 변경하지 않는다.
