...
Observer identity와 combatant identity는 분리한다. Public `CombatObserver`에 explicit `combatant_id`를 추가하고, public `CombatOpportunityContext`에 `combatant_ids: Vec<String>`을 추가한다. Vec는 serde ingress에서도 duplicate 증거와 caller order를 보존한다. S1a에서 `CombatOpportunityCatalog::evaluate(instances, context)` signature는 넓히지 않는다. Evaluate가 호출하는 local context validation은 canonicalization 전에 empty vector, blank/whitespace-only ID, duplicate ID를 reject하며 sort+dedupe로 오류를 숨기지 않는다. 검증을 통과한 뒤에만 stable ID 오름차순 clone을 만들어 fingerprint와 observer membership validation에 사용한다. `observer.id`는 detection/opportunity provenance ID이고 executor/target ID가 아니며, 두 문자열이 우연히 같아도 equality를 mapping으로 사용하지 않는다. Local validation은 observer ID와 mapped combatant ID의 blank, validated context roster non-membership, duplicate observer ID, 둘 이상의 observer가 같은 combatant로 매핑되는 경우를 reject한다. Actual combat request가 evaluate input에 없으므로 S1a local validation은 context roster가 실제 participant roster와 같은지 주장하지 않는다. `observer` executor는 exact opportunity observer의 mapped combatant다. `any_capable`은 observer의 `can_act`와 required capabilities, mapped combatant의 pause-active와 hp>0을 모두 만족하는 후보만 남기고 mapped combatant ID 오름차순으로 exactly one을 고른다. Capability는 observer record, static membership은 validated context roster, active/health와 actual request roster equality는 S1b runtime owner다. 새 entity registry나 authoring identity 의미는 만들지 않는다.

전체 roster는 `CombatOpportunityCandidate`에 복제하지 않는다. 대신 public serialized `CombatOpportunityEvaluation`에 `context_combatant_ids: Vec<String>`을 추가한다. Normal evaluation creation에서는 `evaluate`만 local validation 뒤 만든 stable-ID-sorted context roster clone으로 이 field를 채운다. Field에는 serde default를 두지 않아 누락 JSON은 deserialization에서 실패한다. Present하지만 empty/blank/non-strict-ascending/duplicate인 Vec는 duplicate evidence를 보존한 채 deserialize될 수 있으나 evaluation validation에서 reject한다. `CombatOpportunityEvaluation::canonical_json()`과 기존 no-argument `fingerprint()`는 먼저 이 validation을 실행하고, 성공한 field를 canonical JSON에 포함한다. Fingerprint method에 context argument를 추가하지 않고 mutable external context에도 의존하지 않는다. 따라서 unique context roster 입력 순서 차이는 fingerprint에 영향이 없고, 같은 candidate가 선택되더라도 roster membership이 하나라도 달라지면 fingerprint는 달라지며, duplicate context 입력은 evaluation 생성까지 도달하지 못한다.

Actual roster equality gate는 S1b의 `CombatRuntime`이 소유한다. `CombatRuntime::with_opportunities(request, config)`는 request participant IDs에서 canonical sorted Vec를 직접 derive할 수 있는 constructor path에서는 그 값을 context에 공급하고, caller-provided context를 받는 path에서는 validated sorted context clone과 request participant ID set의 exact equality를 **catalog evaluation 전에** 검사한다. Restore는 checkpoint의 `CombatResolutionRequest`, restored opportunity context의 validated sorted clone, paused evaluation의 canonical `context_combatant_ids`를 모두 exact-equal 비교한 뒤에만 runtime을 받아들이고, 그 뒤 기존 no-argument `pause.evaluation.fingerprint()`를 재계산한다. Evaluation roster의 누락/noncanonical form 또는 세 roster 중 mismatch는 evaluation/restore state acceptance 전에 `InvalidInput`으로 reject한다. 이 gate는 active/hp filtering과 별개다.
...
1. **I2b-S1a — occurrence/identity carrier**: 선행 조건은 PR #219의 I2 prerequisite authoring contract merge다. Owned files는 `crates/escape-core/src/combat_opportunity.rs`, `crates/escape-core/tests/combat_opportunity_wave1.rs`, 그리고 mandatory field 추가로 깨지는 struct literal만 고치는 `crates/escape-core/src/combat_runtime.rs`의 compile fixture/test 영역이다. `CombatOpportunityInstance.bound_target_ids`/`bound_target_tick`, candidate exact copy/fingerprint, `CombatObserver.combatant_id`, `CombatOpportunityContext.combatant_ids: Vec<String>`, `CombatOpportunityEvaluation.context_combatant_ids: Vec<String>`, stable mapped-combatant selection을 구현한다. Repo 전체의 기존 context/evaluation struct literal compile fix는 S1a owner가 함께 수행하되 roster field 주입 외 behavior 변경, evaluate signature 확대, fingerprint method argument 추가는 금지한다. Acceptance는 roster empty/blank/duplicate pre-canonicalization reject와 no-silent-dedupe, observer mapped ID context-membership reject, evaluation roster required/nonblank/strict-sorted/unique validation, canonical JSON inclusion, no-argument fingerprint self-containment, unique roster order-variation fingerprint parity와 membership-change fingerprint delta, bound vector order-independent canonical fingerprint, bound value/tick change fingerprint delta, observer vector reversal parity, capability/can_act filtering이다. Stop은 content authoring schema 변경, participant/entity registry 생성, runtime checkpoint/lifecycle/transaction mutation이 필요할 때다. Forbidden files는 `save.rs`, `combat_intervention_transaction.rs`, TS/Web/generated/storypack, `docs/content/design_source/schema/combat_intervention_response.schema.json`이다.
2. **I2b-S1b — runtime roster gate, pause provenance/checkpoint v2**: I2b-S1a merge와 독립 review가 선행 조건이다. Owned file은 `crates/escape-core/src/combat_runtime.rs`와 그 module-local/focused runtime tests다. Runtime constructors는 request participants에서 canonical sorted context roster를 직접 derive하고, `with_opportunities`는 context/request roster equality를 catalog evaluation 전에 검증한다. Restore는 evaluation `context_combatant_ids`, restored context roster, request participant roster의 three-way exact equality와 canonical form을 fingerprint 재계산/restore acceptance 전에 검증한다. 이어 tick resolution 직전 snapshot, stable pause ID, selected-target exact-one preflight, same-tick KO boundary, paused checkpoint v2 roundtrip/restore validation을 구현한다. Acceptance는 with-opportunities roster mismatch pre-evaluation reject, evaluation roster missing/noncanonical restore reject, three-way restore roster mismatch reject, restored no-argument fingerprint parity, no source·multi source·unknown·prior-KO reject, same-tick lethal accept, AI current target/last move fallback 부재, malformed provenance reject, v2 JSON roundtrip, provenance-less paused v1 explicit reject, preflight 실패 전후 state/cost/RNG/history equality다. Stop은 GameState mutation, terminal/lifecycle settlement, response transaction, `SaveEnvelope` bump, authoring schema 변경이 필요할 때다. Forbidden files는 `save.rs`, `combat_intervention_transaction.rs`, `lib.rs` public facade, TS/Web/generated/storypack, 모든 authoring schema다.
