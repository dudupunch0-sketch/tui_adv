...
| `wuxia_black_heaven_escape_price` | `route_commitment` | `faction_negotiation`, `sect_courtyard`, `market_street` | direct/deferred 사파 route starter를 받아 흑천련 탈출로와 도월의 표식이 남기는 값을 첫 사파 opener로 고정한다. | 다음 runtime handoff |
...
10. `wuxia_black_heaven_escape_price` — docs-only handoff 완료, 다음 runtime 후보
