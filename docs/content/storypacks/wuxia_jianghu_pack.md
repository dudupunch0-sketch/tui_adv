...
- `wuxia_mumyeong_followup_after_midgame_reunion`에서 `wuxia_mumyeong_departure_truth_summary`, `wuxia_boss_first_appearance`, `wuxia_qingliu_attack_after_war`를 우선 비교한다.
- 다음 handoff는 runtime YAML/Rust/Web generated preview artifact를 바로 수정하지 않고, truth reveal/boss wall/full flashback을 열지 먼저 문서에서 결정한다.
