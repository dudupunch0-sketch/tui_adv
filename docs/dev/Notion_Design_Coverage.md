...
Runtime status: `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt`, `wuxia_black_heaven_escape_price`, `wuxia_heavenly_archive_previous_outsiders`, and `wuxia_wounded_shelter_dawn_offers` were implemented later as separate `storypack_preview` runtime slices. The next handoff is the docs-only `route_midgame_continuity_after_wounded_shelter` selection. This document remains the Notion coverage ledger. 2026-06-01 default 전환 이후 Web player 기본 storypack은 이구학지다.
