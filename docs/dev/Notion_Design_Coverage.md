...
Status: docs-only sync + post-confrontation handoff status note
Last checked: 2026-06-02T01:26:59+09:00
Runtime status: `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt`, `wuxia_black_heaven_escape_price`, `wuxia_heavenly_archive_previous_outsiders`, `wuxia_wounded_shelter_dawn_offers`, `wuxia_mumyeong_first_sighting`, and `wuxia_mumyeong_first_confrontation` were implemented later as separate `storypack_preview` runtime slices. The post-confrontation handoff selected `wuxia_mumyeong_copy_style_reveal` as the next runtime candidate. This document remains the Notion coverage ledger. 2026-06-01 default 전환 이후 Web player 기본 storypack은 이구학지다.
...
| 6 | `wuxia_mumyeong_copy_style_reveal` | 무명의 카피 무공 공개 | `wuxia_mumyeong_copy_style_reveal` | docs-only handoff ready; next runtime candidate |
