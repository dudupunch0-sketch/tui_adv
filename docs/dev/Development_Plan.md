...
1. **현재 최우선: Event/Stage/ContentBlock 전환.** 용어·authoring 계약은 `docs/design/Event_Stage_Content_Model.md`로 확정했다. 다음 구현 slice는 기존 Encounter selection 의미를 유지한 채 ordered Stage/ContentBlock schema와 legacy adapter/validation을 추가하는 것이다. 이어서 Event 내부 cursor/save migration → renderer-neutral ordered stream → Web Storybook 고정 5영역 제거 → 이구학지 대표 Event 점진 마이그레이션 순서로 진행한다. 각 slice는 기존 save와 office/wuxia surface parity를 보존하고 독립 검증한다.
