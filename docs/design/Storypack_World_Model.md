...
- 런타임 구현: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt`, `wuxia_black_heaven_escape_price`, `wuxia_heavenly_archive_previous_outsiders`, `wuxia_wounded_shelter_dawn_offers` preview는 완료했고, `yageunmong_pack` runtime은 미착수다.
- 다음 추천: `wuxia_mumyeong_first_sighting` preview runtime slice를 구현한다. `route_midgame_continuity_after_wounded_shelter` handoff에서 공통 post-opener midgame bridge로 골랐으며, 기본 office bundle/legacy `escape-office` key/천외편린 3택 성장/return system/faction route graph/debt ledger schema는 변경하지 않는다.
