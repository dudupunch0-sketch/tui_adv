...
- 첫 비-office 기준팩: `wuxia_jianghu_pack` / **이구학지 — 천기록**. 전제는 “현대 회사원이 본인 몸과 출근복장 그대로 무협 세계 시장에 전이되고, 천기록/천외편린 성장 구조를 경험한다”. 현재 preview는 arrival/first fight/first fragment까지 구현됐고, 다음 구현 slice는 `wuxia_seo_harin_rescue`다. 후속 `wuxia_cheongryu_apprentice_entry`, later `wuxia_cheongryu_raid_route_split`, deferred `wuxia_cheongryu_raid_wounded_fallback`는 prerequisite hook 이후 구현할 설계 후보로 정리했다.
- 1차 재난 타입: 불명 재난
