...
| `wuxia_heavenly_archive_previous_outsiders` | `route_commitment` / `cheonggi_return` | `cheonggi_record`, `faction_negotiation`, `sect_courtyard` | direct/deferred 천기·귀환 route starter를 받아 천기각 이전 이방인 기록과 세계 균열 단서를 첫 천기 opener로 고정한다. | 다음 runtime handoff |
...
11. `wuxia_heavenly_archive_previous_outsiders` — 다음 runtime handoff
   - `route_opener_followup_after_black_heaven` handoff에서 천기·귀환 opener로 결정했다.
