...
docs/
  ...
  runtime preview sources/
    src/tui_adv/storypack-previews/wuxia_jianghu_pack/ # wuxia_commute_rift_arrival부터 wuxia_mumyeong_awakening까지 preview source YAML; 다음은 wuxia_qingliu_attack_after_war runtime 구현
...
2. `wuxia_jianghu_pack` / 이구학지 — 천기록 preview는 `wuxia_mumyeong_awakening`까지 구현 완료. 다음은 `wuxia_qingliu_attack_after_war` runtime 구현
