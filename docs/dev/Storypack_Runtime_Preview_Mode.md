...
Status: 결정 문서 + `wuxia_wounded_shelter_dawn_offers` runtime 구현 완료 + `wuxia_mumyeong_first_sighting` handoff 완료
...
`wuxia_mumyeong_first_sighting` — 다음 runtime handoff 완료. 세 route opener 이후 첫 common midgame bridge로, Notion 사건 카드 DB `무명 첫 목격`을 받아 무명 존재/서하린 침묵/청류문식 카피 무공의 이질감을 암시한다. 다음 구현은 세 route opener outcome에 공통 `route_opener_resolved` flag를 추가하고, `route_opener_resolved` + `cheongryu_raid_survived` + `cheongryu_trial_started` + `first_fragment_seen`를 required flags로 사용한다. `righteous_route_opened`, `sapa_route_opened`, `cheonggi_return_route_opened`는 flavor hook으로만 읽고, any-of condition schema, route graph, faction reputation, relation/debt ledger, combat schema, boss combat, reward/ability/epilogue/return system, 천기록 정체 reveal은 열지 않는다.
