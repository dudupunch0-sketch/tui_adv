...
runtime_promotion_notes: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt`, `wuxia_black_heaven_escape_price`, `wuxia_heavenly_archive_previous_outsiders`, `wuxia_wounded_shelter_dawn_offers`, `wuxia_mumyeong_first_sighting`, `wuxia_mumyeong_first_confrontation`, `wuxia_mumyeong_copy_style_reveal`, `wuxia_mumyeong_reads_orthodox_style`, `wuxia_mumyeong_midgame_reunion`, `wuxia_boss_first_appearance`, `wuxia_mumyeong_request_for_aid`, `wuxia_mumyeong_awakening`, `wuxia_qingliu_attack_after_war`, `wuxia_mumyeong_destroys_orthodox_sect`, `wuxia_boss_recruits_mumyeong`, `wuxia_mumyeong_departure_truth_summary`, `wuxia_seoharin_empty_place`, `wuxia_seoharin_left_meal`, `wuxia_sado_final_phase_1_price_tag`, `wuxia_sado_final_phase_2_weakpoint_control`, `wuxia_sado_final_phase_3_outside_calculation`, `wuxia_boss_resolution`, `wuxia_mumyeong_resolution`, `wuxia_seoharin_qingliu_resolution`, `wuxia_cheongirok_resolution` runtime은 separate storypack preview bundle에서 완료했다. `wuxia_qingliu_attack_after_war` 구현 slice는 full flashback이 아니라 현악문/복호금쇄수 흔적 조사로 landing했고, `wuxia_mumyeong_destroys_orthodox_sect` 구현 slice는 playable 멸문 전투가 아니라 빈 현악문 산문/기록/풍문 trace로 landing했으며, `wuxia_boss_recruits_mumyeong` 구현 slice는 구원/최종 결산이 아니라 흑사방 보스의 recruitment trace로 landing했고, `wuxia_mumyeong_departure_truth_summary` 구현 slice는 서하린에게 진실을 전달하지 않는 sealed summary로 landing했고, `wuxia_seoharin_empty_place` 구현 slice는 truth delivery 없이 서하린의 빈 자리를 late bridge로 남겼고, `wuxia_seoharin_left_meal` 구현 slice는 귀환/정착 최종 분기 없이 서하린의 daily-care belonging bridge로 landing했으며, `wuxia_sado_final_phase_1_price_tag` 구현 slice는 combat resolver 없이 최종장 장부/증거/압박/item-log 상태 씨앗만 landing했고, `wuxia_sado_final_phase_2_weakpoint_control` 구현 slice는 combat resolver 없이 서하린/무명/청류문/천기록 약점 압박 대응 씨앗만 landing했고, `wuxia_sado_final_phase_3_outside_calculation` 구현 slice는 combat resolver 없이 보스 결산 후보 상태 씨앗만 landing했으며, `wuxia_boss_resolution` 구현 slice는 final epilogue renderer 없이 보스 결산 route와 후속 epilogue candidate seed만 landing했고, `wuxia_mumyeong_resolution` 구현 slice는 final epilogue renderer 없이 무명 결산 route와 후속 epilogue candidate seed만 landing했고, `wuxia_seoharin_qingliu_resolution` 구현 slice는 final epilogue renderer 없이 서하린/청류문 결산 route와 후속 epilogue candidate seed만 landing했으며, `wuxia_cheongirok_resolution` 구현 slice는 final epilogue renderer 없이 천기록 마지막 장의 high-use/blank/corruption/low-use/method-reflection 후보 seed만 landing했다. 다음 handoff 후보는 `wuxia_black_serpent_aftermath`이다. 2026-06-01 default 전환 이후 Web player 기본 경로는 이구학지이며, 2026-06-02 terminal content scene default도 이구학지 fixture로 맞췄다. office content는 legacy/parity fixture로 남긴다. legacy office bundle, legacy `escape-office` save/localStorage key, 천외편린 3택 성장 schema, faction/relation/debt ledgers는 아직 열지 않는다.

runtime_history_note: `wuxia_mumyeong_first_sighting`, `wuxia_mumyeong_first_confrontation`, `wuxia_mumyeong_copy_style_reveal`, `wuxia_mumyeong_reads_orthodox_style`, `wuxia_mumyeong_midgame_reunion`, `wuxia_boss_first_appearance`, `wuxia_mumyeong_request_for_aid`, `wuxia_mumyeong_awakening`, `wuxia_qingliu_attack_after_war`, `wuxia_mumyeong_destroys_orthodox_sect`, `wuxia_boss_recruits_mumyeong`, `wuxia_mumyeong_departure_truth_summary`, `wuxia_seoharin_empty_place`, `wuxia_seoharin_left_meal`, `wuxia_sado_final_phase_1_price_tag`, `wuxia_sado_final_phase_2_weakpoint_control`, `wuxia_sado_final_phase_3_outside_calculation`, `wuxia_boss_resolution`, `wuxia_mumyeong_resolution`, `wuxia_seoharin_qingliu_resolution`, `wuxia_cheongirok_resolution` runtime은 separate storypack preview bundle에서 완료했다. `wuxia_cheongirok_resolution_handoff`은 기존 encounter hooks만으로 천기록 마지막 장 route seed bridge를 구현했고, 다음 handoff/runtime 후보는 `wuxia_black_serpent_aftermath`이다.
