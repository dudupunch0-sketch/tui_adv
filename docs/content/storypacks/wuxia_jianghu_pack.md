...
| `wuxia_boss_first_appearance` | `midgame_boss` / `boss_wall_pressure` | `sect_courtyard`, `faction_negotiation`, `training_chore` | 무명 중반 재회 뒤 흑사방 보스가 처음 직접 압박으로 등장하고, 주인공이 흐름을 읽어도 몸이 따라가지 못하는 벽을 각인한다. | 다음 runtime 후보 |
...
18. `wuxia_boss_first_appearance` — 다음 runtime 후보
   - `wuxia_mumyeong_followup_after_midgame_reunion` handoff에서 다음 runtime 후보로 결정했다.
...
- `wuxia_boss_first_appearance`를 `wuxia_mumyeong_midgame_reunion` 뒤에 storypack preview source로 추가한다.
- runtime YAML/Rust/Web generated preview artifact는 다음 구현 slice에서만 수정한다.
