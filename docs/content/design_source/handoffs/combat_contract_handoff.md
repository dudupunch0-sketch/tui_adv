...
Observer identity와 combatant identity는 분리한다. Public `CombatObserver`에 explicit `combatant_id`를 추가하고, public `CombatOpportunityContext`에 `combatant_ids: Vec<String>`을 추가한다. Vec는 serde ingress에서도 duplicate 증거와 caller order를 보존한다. `CombatOpportunityCatalog::evaluate`가 호출하는 context validation은 canonicalization 전에 empty vector, blank/whitespace-only ID, duplicate ID를 reject하며 sort+dedupe로 오류를 숨기지 않는다. 검증을 통과한 뒤에만 stable ID 오름차순 clone을 만들고, 이 clone이 해당 actual combat request의 pause/static participant roster 전체 ID set과 exact-equal한지 비교한다. `combatant_ids`는 active/hp 후보 목록이 아니다. Runtime constructors는 처음부터 canonical sorted IDs를 전달해야 하지만 public API는 order variation을 허용하며 같은 unique membership이면 같은 결과를 낸다. `observer.id`는 detection/opportunity provenance ID이고 executor/target ID가 아니며, 두 문자열이 우연히 같아도 equality를 mapping으로 사용하지 않는다. Opportunity context validation은 observer ID와 mapped combatant ID의 blank, validated roster non-membership, duplicate observer ID, 둘 이상의 observer가 같은 combatant로 매핑되는 경우를 reject한다. `observer` executor는 exact opportunity observer의 mapped combatant다. `any_capable`은 observer의 `can_act`와 required capabilities, mapped combatant의 pause-active와 hp>0을 모두 만족하는 후보만 남기고 mapped combatant ID 오름차순으로 exactly one을 고른다. Capability는 observer record, static membership은 validated context roster, active/health는 S1b의 combat runtime snapshot이 owner다. 새 entity registry나 authoring identity 의미는 만들지 않는다.

전체 roster는 `CombatOpportunityCandidate`에 복제하지 않는다. Exact roster comparison과 evaluation/pause fingerprint canonical input은 모두 validation 뒤 만든 같은 stable-ID-sorted clone을 사용한다. Fingerprint는 evaluation payload 뒤에 이 clone을 별도 항목으로 포함한다. 따라서 unique roster 입력 순서 차이는 fingerprint에 영향이 없고, 같은 candidate가 선택되더라도 roster membership이 하나라도 달라지면 fingerprint는 달라지며, duplicate 입력은 fingerprint 단계까지 도달하지 못한다. Candidate canonical JSON만으로 roster-aware fingerprint를 대신해서는 안 된다.
