...
docs/
  ...
  runtime preview sources/
    src/tui_adv/storypack-previews/wuxia_jianghu_pack/ # wuxia_commute_rift_arrival부터 wuxia_mumyeong_request_for_aid까지 preview source YAML; 다음은 wuxia_mumyeong_awakening runtime implementation
...
2. `wuxia_jianghu_pack` / 이구학지 — 천기록 preview는 `wuxia_mumyeong_request_for_aid`까지 구현 완료. `wuxia_mumyeong_followup_after_failed_aid` handoff가 `wuxia_mumyeong_awakening`을 다음 runtime 후보로 선택함
