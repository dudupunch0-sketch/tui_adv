...
| `wuxia_mumyeong_midgame_reunion` | `midgame_rival` / `rival_reunion` | `sect_courtyard`, `cheonggi_record`, `training_chore` | 첫 대치/카피 무공/정파식 통제 무공 단서 뒤, 무명과 다시 마주쳐 서하린의 침묵과 과거 상처를 라이벌/거울 관계로 연결한다. | 다음 runtime 후보 |
...
17. `wuxia_mumyeong_midgame_reunion` — docs-only handoff 완료 / 다음 runtime 후보
   - `wuxia_mumyeong_followup_after_orthodox_style_trace` handoff에서 다음 runtime 후보로 결정했다.
...
- `wuxia_mumyeong_midgame_reunion`을 `wuxia_mumyeong_reads_orthodox_style` 뒤에 추가한다.
- 다음 구현은 runtime YAML/Rust/Web generated preview artifact를 수정하지만, legacy office bundle/default legacy generated content는 바꾸지 않는다.
