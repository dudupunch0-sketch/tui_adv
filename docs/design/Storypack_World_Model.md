...
- 기본 storypack: `office_isolation_pack` / `isolation_pack` 계열. 기존 문서와 런타임 콘텐츠의 현재 구현체다.
...
- `docs/content/storypack_db/encounter_situations.json`에 `isolation_pack` 6개, `yageunmong_pack` 6개, `wuxia_jianghu_pack` 7개, 총 19개 repo 후보 카드를 둔다. Notion 사건 카드 DB의 26개 row는 `docs/dev/Notion_Design_Coverage.md`에서 매핑한 upstream source이며, machine-readable mirror에 전부 자동 import하지 않는다.
...
- `wuxia_jianghu_pack` runtime 후보는 explicit preview bundle 또는 preview flag로만 연다.
...
진행 상태: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment` preview는 완료했다. 다음 구현 slice는 `wuxia_seo_harin_rescue`로 확정했고, `wuxia_cheongryu_apprentice_entry`는 설계/handoff 완료된 그 다음 bridge 후보다. `wuxia_cheongryu_raid_route_split`과 `wuxia_cheongryu_raid_wounded_fallback`도 설계 완료했지만 rescue/apprentice/first-fragment/raid hook 뒤에만 여는 later candidates다.

- 세 preview encounter는 같은 `storypack_preview` bundle에만 둔다.
- 다음 `wuxia_seo_harin_rescue` slice도 기존 encounter schema로 넣는다: `conditions`, `choices`, `cost`, `outcome.resources`, `danger`, `add_flags`, `add_clues`, `add_items`/`remove_items`, `destination_id`, `log`, optional `presentation`.
...
- `wuxia_cheongryu_apprentice_entry`의 안정 choice id는 `accept_three_month_trial`(fallback), `request_martial_training_immediately`, `organize_chores_like_workflow`, `inspect_archive_during_chore`다.
...
- 상태: 설계 문서화 + machine-readable storypack DB 검증 + preview mode 결정 + 무협 세 preview runtime + preview launcher/UI wiring 완료. 다음 `wuxia_seo_harin_rescue` slice, 후속 `wuxia_cheongryu_apprentice_entry`, later `wuxia_cheongryu_raid_route_split`, deferred `wuxia_cheongryu_raid_wounded_fallback` 후보 설계/handoff까지 완료했다.
- 기본 office 계열: `isolation_pack`은 현재 runtime에 가까운 기본 후보, `yageunmong_pack`은 별도 office-dream 후보.
- 첫 비-office storypack: `wuxia_jianghu_pack` / **이구학지 — 천기록**.
- 이전 무협 placeholder: superseded.
- 런타임 구현: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment` preview는 완료했고, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `yageunmong_pack` runtime은 미착수다.
- 다음 추천: `wuxia_seo_harin_rescue`를 같은 `storypack_preview` bundle에 먼저 구현한다. 그 다음 설계 완료된 `wuxia_cheongryu_apprentice_entry`를 `seo_harin_rescue_resolved`/`taken_under_watch` hook으로 이어 붙인다. `wuxia_cheongryu_raid_route_split`는 rescue/apprentice와 `cheonggi_record_awakened`/`first_fragment_seen` hook이 안정화된 뒤에만 열고, `wuxia_cheongryu_raid_wounded_fallback`은 raid split fallback branch 뒤 조건부로 연다. 기본 office bundle/`escape-office` key/천외편린 3택 성장/faction route graph schema는 변경하지 않는다.
