...
- 현재 기본 개발 기준팩: `wuxia_jianghu_pack` / **이구학지 — 천기록**. 전제는 “현대 회사원이 본인 몸과 출근복장 그대로 무협 세계 시장에 전이되고, 천기록/천외편린 성장 구조를 경험한다”. 현재 Web/terminal default storypack이며, runtime은 arrival/first fight/first fragment부터 `wuxia_sado_final_phase_2_weakpoint_control`까지 구현됐다. `wuxia_sado_final_phase_2_weakpoint_control`은 사도 최종전 2페이즈 약점 장악 slice로, 서하린/무명/청류문/천기록 압박 대응을 final-state seed로 남긴다. 다음 handoff 후보는 `wuxia_sado_final_phase_3_outside_calculation`이다.
- 1차 재난 타입: 불명 재난
...
- `src/tui_adv/storypack-previews/wuxia_jianghu_pack/`: `wuxia_commute_rift_arrival`부터 `wuxia_sado_final_phase_2_weakpoint_control`까지의 이구학지 source YAML
