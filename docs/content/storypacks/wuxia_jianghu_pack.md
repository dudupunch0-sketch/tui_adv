...
- `wuxia_mumyeong_followup_after_copy_style_reveal`에서 `wuxia_mumyeong_midgame_reunion`, `wuxia_boss_first_appearance`, `wuxia_mumyeong_departure_truth_summary`, `wuxia_mumyeong_reads_orthodox_style`를 비교한다.
- 다음 handoff는 runtime YAML/Rust/Web generated artifact를 수정하지 않고, seed 기반 random copy-style system, 천외편린 reward schema, boss combat, route graph/faction reputation/debt/relation ledger, 무명 과거 진실 reveal을 열지 말지 먼저 결정한다.
