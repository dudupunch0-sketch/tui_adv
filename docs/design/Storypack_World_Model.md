...
- Notion 사건 카드 DB 26개와 후일담 카드 DB 17개는 future/source coverage로 추적하되, 현재 runtime/machine-readable mirror에 자동으로 모두 반영하지 않는다. Repo mirror는 runtime 승격된 final-entry 카드까지 포함해 `wuxia_jianghu_pack` 28개 후보 카드를 둔다.
...
- `docs/content/storypack_db/encounter_situations.json`에 `isolation_pack` 6개, `yageunmong_pack` 12개, `wuxia_jianghu_pack` 28개, 총 46개 repo 후보 카드를 둔다. Notion 사건 카드 DB의 26개 row는 `docs/dev/Notion_Design_Coverage.md`에서 매핑한 upstream source이며, machine-readable mirror에 전부 자동 import하지 않는다.
...
- 상태: 설계 문서화 + machine-readable storypack DB 검증 + preview mode 결정 + 무협 스물여덟 개 preview runtime + Web/terminal default 이구학지 wiring 완료.
...
- 런타임 구현: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt`, `wuxia_black_heaven_escape_price`, `wuxia_heavenly_archive_previous_outsiders`, `wuxia_wounded_shelter_dawn_offers`, `wuxia_mumyeong_first_sighting`, `wuxia_mumyeong_first_confrontation`, `wuxia_mumyeong_copy_style_reveal`, `wuxia_mumyeong_reads_orthodox_style`, `wuxia_mumyeong_midgame_reunion`, `wuxia_boss_first_appearance`, `wuxia_mumyeong_request_for_aid`, `wuxia_mumyeong_awakening`, `wuxia_qingliu_attack_after_war`, `wuxia_mumyeong_destroys_orthodox_sect`, `wuxia_boss_recruits_mumyeong`, `wuxia_mumyeong_departure_truth_summary`, `wuxia_seoharin_empty_place`, `wuxia_seoharin_left_meal`, `wuxia_sado_final_phase_1_price_tag`, `wuxia_sado_final_phase_2_weakpoint_control` preview는 완료했다. `wuxia_sado_final_phase_1_price_tag`는 final state routing contract 뒤 첫 price-tag/ledger phase로 구현됐고, `wuxia_sado_final_phase_2_weakpoint_control`은 phase 1 seed를 이어 받아 약점 압박 대응을 state seed로 남기는 phase로 구현됐다. `yageunmong_pack` runtime은 미착수다.
- 다음 추천: `wuxia_sado_final_phase_3_outside_calculation` handoff/runtime candidate 검토. `docs/design/Wuxia_Final_State_Routing.md`의 final-state contract를 기준으로 phase 2 seed를 이어받되, 서하린 truth delivery, `told_seoharin_truth`, 무명 구원 확정, 무명/보스 결산, epilogue/return, legacy office bundle/legacy `escape-office` key/seed 기반 random copy-style system/천외편린 3택 성장/faction route graph/debt ledger/combat schema/boss combat/final boss resolution은 바로 변경하지 않는다.
