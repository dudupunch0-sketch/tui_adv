...
- 다음 추천: `wuxia_seoharin_left_meal_followup` docs-only handoff. final/late companion 후보를 비교하되 서하린 truth delivery, `told_seoharin_truth`, 무명 구원 확정, 무명/보스 결산, epilogue/return, legacy office bundle/legacy `escape-office` key/seed 기반 random copy-style system/천외편린 3택 성장/faction route graph/debt ledger/combat schema/boss combat/final boss resolution은 바로 변경하지 않는다.
