...
def test_wuxia_cheongryu_raid_wounded_fallback_runtime_slice_is_docs_synced():
    ...
    assert "current_goal: wuxia_mumyeong_awakening" in next_goal
...
def test_wuxia_baekdo_medicine_debt_runtime_slice_is_docs_synced():
    ...
    assert "- `wuxia_jianghu_pack`: 이구학지 — 천기록 후보 카드 20개." in storypack_db_readme
...
def test_wuxia_mumyeong_awakening_handoff_is_docs_synced():
    plan = Path("docs/dev/Development_Plan.md").read_text(encoding="utf-8")
    checklist = Path("docs/dev/Checklist.md").read_text(encoding="utf-8")
    coverage = Path("docs/dev/Notion_Design_Coverage.md").read_text(encoding="utf-8")
    next_goal = Path("idea_box/next_goal/README.md").read_text(encoding="utf-8")
    ...
    assert (
        ...
        "`wuxia_mumyeong_awakening` | next runtime selected |"
    ) in coverage
    assert "current_goal: wuxia_mumyeong_awakening" in next_goal
    assert "mode: implementation" in next_goal
    assert "encounter id: `wuxia_mumyeong_awakening`" in next_goal
    assert "compare_anger_to_copied_flow" in next_goal
    assert "trace_awakening_from_failed_aid" in next_goal
    assert "ask_what_the_copy_cost_him" in next_goal
    assert "stop_before_calling_it_salvation" in next_goal
    assert "- `wuxia_jianghu_pack`: 이구학지 — 천기록 후보 카드 20개." in storypack_db_readme
    ...
    assert "20. `wuxia_mumyeong_awakening` — 다음 runtime 후보" in wuxia_pack
    ...
    assert "wuxia_mumyeong_awakening` — 다음 runtime 후보" in runtime_preview
