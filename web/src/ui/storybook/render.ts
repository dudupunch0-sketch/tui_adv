...
function renderHud(page: ScenePage): string {
  ...
  return `<header class="storybook-hud" data-region="status" data-danger-band="${dangerBand(page.status_summary.danger)}">
    ...
    <p class="hud-document" aria-label="현재 기록 ${escapeHtml(documentLabel(page))} · ${page.status_summary.turn}턴" title="${page.status_summary.turn}턴">${escapeHtml(documentLabel(page))} ${page.status_summary.turn}쪽</p>
    ${renderProgressRail(page)}
    <button type="button" class="hud-drawer-toggle" data-player-action="toggle-storybook-drawer" aria-expanded="false" aria-controls="storybook-info-drawer"><span aria-hidden="true">詳</span>상세</button>
  </header>`;
}
