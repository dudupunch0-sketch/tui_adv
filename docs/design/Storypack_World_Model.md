...
- 상태: 설계 문서화 + machine-readable storypack DB 검증 + preview mode 결정 + 무협 열한 개 preview runtime + preview launcher/UI wiring 완료.
...
- 런타임 구현: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt`, `wuxia_black_heaven_escape_price`, `wuxia_heavenly_archive_previous_outsiders` preview는 완료했고, `yageunmong_pack` runtime은 미착수다.
- 다음 추천: `route_opener_followup_after_heavenly_archive`에서 deferred-offer card 또는 세 route opener 이후 midgame continuity 중 하나를 고른다. 기본 office bundle/legacy `escape-office` key/천외편린 3택 성장/return system/faction route graph/debt ledger schema는 변경하지 않는다.
