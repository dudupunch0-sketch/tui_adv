...
- `docs/content/storypack_db/encounter_situations.json`에 `isolation_pack` 6개, `yageunmong_pack` 6개, `wuxia_jianghu_pack` 10개, 총 22개 repo 후보 카드를 둔다. Notion 사건 카드 DB의 26개 row는 `docs/dev/Notion_Design_Coverage.md`에서 매핑한 upstream source이며, machine-readable mirror에 전부 자동 import하지 않는다.
...
진행 상태: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt` preview는 완료했다. `route_opener_followup_after_baekdo` docs-only handoff도 완료했으며, 다음 runtime 후보는 `wuxia_black_heaven_escape_price`다.

- 여덟 preview encounter는 같은 `storypack_preview` bundle에만 둔다.
...
- 상태: 설계 문서화 + machine-readable storypack DB 검증 + preview mode 결정 + 무협 여덟 preview runtime + preview launcher/UI wiring 완료.
...
- 런타임 구현: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt` preview는 완료했고, `yageunmong_pack` runtime은 미착수다.
- 다음 추천: `wuxia_black_heaven_escape_price` runtime slice를 구현한다. direct/deferred branch 차이는 `black_heaven_deal_marked`/`black_heaven_escape_marker` flavor hook으로만 읽고, eligibility는 `sapa_route_started` + `dowol_debt`로 둔다. 기본 office bundle/legacy `escape-office` key/천외편린 3택 성장/faction route graph/debt ledger schema는 변경하지 않는다.
