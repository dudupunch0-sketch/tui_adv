...
- **이 인카운터는 `combat_spectator_preview_unlocked` 게이트 뒤에 있다** — terminal·Web 렌더러 모두 확보됐지만(Step 1d-1, Step 1d-2) 게이트는 아직 제거하지 않았다. Step 1d-3이 재생 연출(모션)을 얹은 뒤에 게이트를 풀어 정식 경로로 승격한다 — Step 1d-1에서 세운 "실시간 연출이 없는 화면을 플레이어 경로에 올리지 않는다"는 판단을 유지한다.

**Wave 3 Step 1d-3 (남은 범위, 별도 플랜 필요)** — Step 1d-2가 확보한 것: 정지 프레임(마지막 `frames` 항목) 렌더, TS 타입 계약, 6개 로그 템플릿, 전투 종료 보고서, 70:30 레이아웃, 접근성 대체 표, forced-colors 대체(`web/src/ui/storybook/combat/renderCombatStage.ts`·`combatLogTemplates.ts`, `web/src/core/types.ts`, `web/src/styles/storybook.css`). Step 1d-3이 그 위에 더해야 할 것:
  - 재생 연출(틱 기반 모션) — 같은 DOM·CSS 변수(`--piece-x`/`--piece-y`) 위에 `@media (prefers-reduced-motion: no-preference)` 안에서만 시간축을 움직인다. `prefers-reduced-motion: reduce`에서는 Step 1d-2가 만든 정지 프레임이 그대로 최종 상태다(재작업 아님).
  - 로그의 실시간 동시 노출 타이밍(현재는 정적 전체 렌더).
  - 게이트 플래그(`combat_spectator_preview_unlocked`) 제거 — 실시간 연출이 갖춰진 뒤에만.
  - wasm 재빌드, 5뷰포트(`web/scripts/storybook-reference-qa.mjs`의 `VIEWPORTS`: 390x844/414x896/800x1440/810x1644/1440x1000) 실화면 QA(`npm run qa:storybook:visual`) — Step 1d-2는 게이트가 남아 있어 시작 화면에서 인카운터에 도달할 수 없으므로 이 QA를 실행할 수 없었다.
  ...
  - **실화면 검증 방법** — 게이트가 남아 있는 동안에는 `npm run qa:storybook:visual`이 이 화면에 도달하지 못한다. Step 1d-2에서는 오케스트레이터가 `web/dist`의 실제 빌드 CSS를 링크한 임시 하네스 HTML을 만들어 320/390/1280에서 실측했고(하네스는 커밋하지 않았다), 그 과정에서 단위 테스트가 잡지 못한 결함 3건(cue 표식 겹침, 적 말 대비 부족, 데스크톱에서 보드 너비가 764px 칸 안에서 420px로 축소)을 찾아 고쳤다. 게이트를 푼 뒤에는 정식 5뷰포트 QA로 대체한다.
...
| (플랜 미작성) — Wave 3 Step 1d-3 | 재생 연출(틱 기반 모션), 로그 동시 노출 타이밍, 게이트 플래그 제거, wasm 재빌드, 5뷰포트 실화면 QA | seed·판정·AI·로그 순서 재구현, 밸런스 확정값 |
