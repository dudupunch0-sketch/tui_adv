...
9. 여러 히든 현실 보물
10. Combat implementation is now tracked by docs/design/Combat_System_Implementation_Plan_Index.md; Wave 1 Step 1~3 and Wave 2 Step 1~4 (fixed integer positions, role/target contract, simultaneous tick frame, execution mode parity and dual logs, data-driven collision/attack/damage/effect resolution, deterministic multi-agent conclusion/termination sidecar) are implemented and verified. Wave 2 Step 4 landed the deterministic conclusion sidecar (`crates/escape-core/src/combat_conclusion.rs`, `crates/escape-core/tests/combat_conclusion_wave2.rs`); early-termination/tick abort, rout/surrender/reinforcement/formation/cohesion/background combat, a combat-end narrative/report consumer, Wave 3 Step 1 spectator adapter, Wave 3 Step 2 authoring, and data-driven balance remain unstarted. Execute one plan file per coding subagent slice; advanced AI behavior, renderer adapter, and balance remain split by approval.
