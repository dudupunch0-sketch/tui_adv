...
fn content_tui_smoke_renders_start_encounter_panel() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("[Terminal Snapshot]"));
    assert!(stdout.contains("ESCAPE OFFICE // TERMINAL EDITION"));
...
fn content_tui_smoke_renders_wuxia_storypack_preview_arrival() {
    ...
    let stdout = String::from_utf8_lossy(&output.stdout);
    assert!(stdout.contains("이구학지 - 천기록 // TERMINAL STORYBOOK"));
    assert!(stdout.contains("위치: 출근길 균열 (wuxia_commute_rift)"));
    assert!(stdout.contains("[이야기]"));
    assert!(stdout.contains("출근길 균열"));
    assert!(stdout.contains("[NO IMAGE] 출근길 균열 너머 낯선 관도에 떨어진 회사원"));
    assert!(stdout.contains("event:continue / 계속"));
...
fn content_tui_smoke_wuxia_settlement_stay_scene_sets_resolved_flag() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_escape-terminal"))
        .args([
            ...
            "choice:turn_the_last_page_without_question",
            "--action",
            "choice:place_the_button_in_the_storage_box",
            "--action",
            "choice:mark_broken_serpent_without_erasing_scars",
        ])
        .output()
