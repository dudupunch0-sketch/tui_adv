...
- `docs/content/storypack_db/encounter_situations.json`: `EncounterSituationCard` 후보 목록. 현재 `isolation_pack` 6개, `yageunmong_pack` 6개, `wuxia_jianghu_pack` 15개, 총 27개 repo 후보 카드를 포함한다. Notion 사건 카드 DB 26개 row는 upstream design source이며, 이 mirror에 자동으로 전부 추가하지 않는다.
...
- `wuxia_heavenly_archive_previous_outsiders`, `wuxia_wounded_shelter_dawn_offers`, `wuxia_mumyeong_first_sighting`, `wuxia_mumyeong_first_confrontation`, `wuxia_mumyeong_copy_style_reveal`, `wuxia_mumyeong_reads_orthodox_style` preview도 완료했다.
...
- `wuxia_mumyeong_followup_after_copy_style_reveal` handoff는 다음 runtime 후보를 `wuxia_mumyeong_reads_orthodox_style`로 결정했고, 현악문/복호금쇄수 clue는 기존 encounter schema로 구현 완료했다. 다음 handoff는 `wuxia_mumyeong_followup_after_orthodox_style_trace` docs-only 후보 비교이며, post-opener any-of condition, route graph, faction reputation, debt/relation/combat/random copy-style/reward schema는 아직 열지 않는다.
