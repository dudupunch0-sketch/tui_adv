...
- `wuxia_jianghu_pack`: 이구학지 — 천기록 후보 카드 19개.
...
`wuxia_mumyeong_reads_orthodox_style`는 preview runtime에 구현됐고, `wuxia_mumyeong_midgame_reunion`는 preview runtime에 구현됐고, `wuxia_boss_first_appearance`는 preview runtime에 구현됐고, `wuxia_mumyeong_request_for_aid`는 preview runtime에 구현됐다. 다음은 `wuxia_mumyeong_followup_after_failed_aid` docs-only handoff다.
