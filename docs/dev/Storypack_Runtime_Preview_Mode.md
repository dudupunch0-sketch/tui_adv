...
- `wuxia_jianghu_pack` is the Web/default storypack through `web/src/core/contentBundles.ts`; terminal still uses the explicit `--storypack-preview wuxia_jianghu_pack` flag until a separate terminal default change is made.
...
- implemented launcher/default wiring: terminal은 `--storypack-preview wuxia_jianghu_pack`, Web 새 게임은 이구학지 default bundle을 사용한다.
