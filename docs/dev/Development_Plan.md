...
1. 무협 storypack preview/main의 다음 작업은 `wuxia_final_state_canonical_collapse_contract_implementation`이다. `wuxia_battle_loss_epilogue_followup_handoff`는 battle-loss runtime evidence 이후 full final battle container, broader corruption/closed-gate branch, reward/ability schema, relation/debt/faction ledger, main ending archive/save surface, playable defeat-route bridge를 비교했고, Notion `사도 최종전 상태값 사전`이 요구하는 local suffix flag collapse를 먼저 열기로 결정했다. 첫 구현은 `crates/escape-core/src/final_epilogue.rs`가 기존 `final_*_seeded` flags를 `combat_result`, `boss_resolution_route`, `evidence_state`, `network_handling`, `pressure_state`, `seoharin_axis`, `qingliu_rebuild`, `mumyeong_salvation`, `successor_route`, `own_flow_choice`, `truth_state`, `cheongirok_state`, `player_method`, `item_logs` 같은 canonical final state labels로 접고, existing `ScenePage.body_blocks`에 `epilogue_state_audit`를 출력하는 계약이다. `wuxia_sado_final_phase_1_price_tag`, `wuxia_sado_final_phase_2_weakpoint_control`, `wuxia_sado_final_phase_3_outside_calculation`, `wuxia_boss_resolution`, `wuxia_mumyeong_resolution`, `wuxia_seoharin_qingliu_resolution`, `wuxia_seoharin_unsaid_stay`, `wuxia_cheongirok_resolution`, `wuxia_black_serpent_aftermath`, `wuxia_final_epilogue_renderer_contract`, final epilogue UX/playtest follow-up, return/settlement contract handoff, return/settlement follow-up handoff, return/settlement epilogue contract runtime, return/settlement epilogue follow-up handoff, battle-loss epilogue contract runtime, battle-loss epilogue follow-up handoff는 완료됐고, `docs/design/Wuxia_Final_State_Routing.md`가 canonical final inputs/result priority/alias policy/final epilogue seed-consumption contract와 return/settlement/battle-loss/final-state-collapse handoff contract를 소유한다.
...
1. 다음 무협 storypack preview/main 작업은 `wuxia_final_state_canonical_collapse_contract_implementation`이다. 이 slice는 `wuxia_battle_loss_epilogue_followup_handoff`가 선택한 다음 runtime으로, 기존 `final_*_seeded` local flags를 Rust GameCore가 canonical final state labels로 collapse하고 `epilogue_state_audit` body block으로 내보내는 계약이다. `wuxia_sado_final_phase_1_price_tag`, `wuxia_sado_final_phase_2_weakpoint_control`, `wuxia_sado_final_phase_3_outside_calculation`, `wuxia_boss_resolution`, `wuxia_mumyeong_resolution`, `wuxia_seoharin_qingliu_resolution`, `wuxia_seoharin_unsaid_stay`, `wuxia_cheongirok_resolution`, `wuxia_black_serpent_aftermath`, `wuxia_final_epilogue_renderer_contract`, `wuxia_return_settlement_epilogue_contract`, `wuxia_battle_loss_epilogue_contract`, final epilogue UX/playtest follow-up, return/settlement contract handoff, return/settlement epilogue follow-up handoff, battle-loss epilogue follow-up handoff는 final state routing contract 기준으로 구현/검증 완료했다.
