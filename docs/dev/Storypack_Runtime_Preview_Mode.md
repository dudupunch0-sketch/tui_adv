...
Status: 결정 문서 + `wuxia_seoharin_left_meal` runtime 구현 완료 + final state routing gate 문서화
...
`wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt`, `wuxia_black_heaven_escape_price`, `wuxia_heavenly_archive_previous_outsiders`, `wuxia_wounded_shelter_dawn_offers`, `wuxia_mumyeong_first_sighting`, `wuxia_mumyeong_first_confrontation`, `wuxia_mumyeong_copy_style_reveal`, `wuxia_mumyeong_reads_orthodox_style`, `wuxia_mumyeong_midgame_reunion`, `wuxia_boss_first_appearance`, `wuxia_mumyeong_request_for_aid`, `wuxia_mumyeong_awakening`, `wuxia_qingliu_attack_after_war`, `wuxia_mumyeong_destroys_orthodox_sect`, `wuxia_boss_recruits_mumyeong`, `wuxia_mumyeong_departure_truth_summary`, `wuxia_seoharin_empty_place`, `wuxia_seoharin_left_meal`는 같은 preview/main storypack 경로에 추가되었다. 이미 preview export/check command, Rust/Web preview bundle artifact, terminal default/explicit `--storypack-preview wuxia_jianghu_pack`, Web default 이구학지 start/save wiring이 있으므로, 다음은 launcher나 천외편린 reward schema가 아니라 `docs/design/Wuxia_Final_State_Routing.md`가 정한 final state routing gate를 기준으로 `wuxia_sado_final_phase_1_price_tag`를 검토하는 것이다.

Latest implemented runtime: `wuxia_seoharin_left_meal`. `wuxia_seoharin_empty_place_followup` handoff에서 선택했고, `wuxia_seoharin_empty_place` 뒤에 삽입했다. 이 slice는 final return/settlement choice가 아니라 `left_meal_memory` layout으로 서하린의 daily-care/청류문 소속감 bridge를 준비한다.

Next design/runtime gate: `docs/design/Wuxia_Final_State_Routing.md`. Latest implemented runtime remains `wuxia_seoharin_left_meal`; the next runtime candidate after the state contract is `wuxia_sado_final_phase_1_price_tag`. It must still avoid truth delivery, `told_seoharin_truth`, Mumyeong salvation confirmation, final resolution, epilogue/return schema, combat resolver, HP numeric battle, route/faction/relation/debt/reward schema, or `item_unpriced_wooden_sword` payout.
