...
- 다음 추천: `route_opener_followup_after_black_heaven`에서 천기·귀환 opener 또는 deferred-offer card 중 하나를 고른다. direct/deferred branch 차이는 flavor hook으로만 읽고, 기본 office bundle/legacy `escape-office` key/천외편린 3택 성장/faction route graph/debt ledger schema는 변경하지 않는다.
