...
- 런타임 구현: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt`, `wuxia_black_heaven_escape_price`, `wuxia_heavenly_archive_previous_outsiders`, `wuxia_wounded_shelter_dawn_offers`, `wuxia_mumyeong_first_sighting`, `wuxia_mumyeong_first_confrontation`, `wuxia_mumyeong_copy_style_reveal`, `wuxia_mumyeong_reads_orthodox_style`, `wuxia_mumyeong_midgame_reunion`, `wuxia_boss_first_appearance`, `wuxia_mumyeong_request_for_aid`, `wuxia_mumyeong_awakening`, `wuxia_qingliu_attack_after_war`, `wuxia_mumyeong_destroys_orthodox_sect`, `wuxia_boss_recruits_mumyeong`, `wuxia_mumyeong_departure_truth_summary` preview는 완료했다. `wuxia_mumyeong_first_sighting`는 `route_midgame_continuity_after_wounded_shelter` handoff 결과로 구현됐고, `wuxia_mumyeong_first_confrontation`는 `wuxia_mumyeong_first_confrontation_after_sighting` handoff 결과로 구현됐다. `wuxia_mumyeong_reads_orthodox_style`는 `wuxia_mumyeong_followup_after_copy_style_reveal` handoff 결과로 구현됐다. `wuxia_mumyeong_midgame_reunion`는 `wuxia_mumyeong_followup_after_orthodox_style_trace` handoff 결과로 구현됐다. `wuxia_boss_first_appearance`는 `wuxia_mumyeong_followup_after_midgame_reunion` handoff 결과로 구현됐고, `wuxia_mumyeong_request_for_aid`는 `wuxia_boss_followup_after_first_appearance` handoff 결과로 구현됐으며, `wuxia_mumyeong_awakening`은 `wuxia_mumyeong_followup_after_failed_aid` handoff 결과로 구현됐다. `wuxia_mumyeong_departure_truth_summary_followup` docs-only handoff는 다음 후보를 `wuxia_seoharin_empty_place`로 결정했다. `yageunmong_pack` runtime은 미착수다.
- 다음 추천: `wuxia_seoharin_empty_place` runtime implementation. sealed truth summary 뒤의 empty-place bridge로만 구현하고, 서하린 truth delivery, `told_seoharin_truth`, 무명 구원 확정, 무명/보스 결산, epilogue/return, legacy office bundle/legacy `escape-office` key/seed 기반 random copy-style system/천외편린 3택 성장/faction route graph/debt ledger/combat schema/boss combat/final boss resolution은 바로 변경하지 않는다.
