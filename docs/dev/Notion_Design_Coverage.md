...
Status: docs-only sync
Last checked: 2026-06-01T02:53:42+00:00
Runtime status: not changed by this document
...
| 6 | `wuxia_mumyeong_copy_style_reveal` | 무명의 카피 무공 공개 | none yet | future rival/growth event source |
| 7 | `wuxia_mumyeong_resolution` | 무명 결산 | none yet | future final-route event source |
| 8 | `wuxia_mumyeong_midgame_reunion` | 무명 중반 재회 | none yet | future midgame rival event source |
| 9 | `wuxia_boss_resolution` | 보스 결산 | none yet | future final boss/result event source |
| 10 | `wuxia_mumyeong_first_sighting` | 무명 첫 목격 | none yet | future midgame reveal source |
| 11 | `wuxia_mumyeong_first_confrontation` | 무명 첫 대치 | none yet | future rival fight source |
| 12 | `wuxia_boss_first_appearance` | 보스 첫 등장 | none yet | future boss-wall source |
...
| 16 | `wuxia_mumyeong_reads_orthodox_style` | 무명의 정파 무공 간파 | none yet | future 청류안 contrast source |
| 17 | `wuxia_qingliu_attack_after_war` | 무너져가는 청류문 습격 | none yet | future backstory/route pressure source |
| 18 | `wuxia_mumyeong_request_for_aid` | 무명의 도움 요청 | none yet | future backstory/source row |
...
| 20 | `wuxia_seoharin_intervention` | 서하린의 개입 | `wuxia_seo_harin_rescue` | next runtime slice; docs/handoff ready; not runtime implemented |
| 21 | `wuxia_prologue_commute_rift` | 출근길의 균열 | `wuxia_commute_rift_arrival` | preview runtime implemented |
| 22 | `wuxia_qingliu_apprentice_entry` | 청류문 임시 수습생 등록 | `wuxia_cheongryu_apprentice_entry` | docs/handoff ready follow-up; not runtime implemented |
| 23 | `wuxia_qingliu_first_arrival` | 청류문 첫 도착 | partial: `wuxia_cheongryu_apprentice_entry` / `cheongryu_outer_courtyard` | future location/arrival beat folded into apprentice handoff for now |
...
- `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, and `wuxia_cheonggi_record_first_fragment` exist as separate storypack preview runtime content.
- `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_raid_route_split`, and `wuxia_cheongryu_raid_wounded_fallback` were already designed as docs/handoff candidates, not runtime content.
...
- `서하린의 개입` mapped to repo `wuxia_seo_harin_rescue` and kept as the next implementation slice.

Still not runtime implemented:

- `wuxia_seo_harin_rescue` and all later repo 후보 cards.
- Most Notion event DB rows beyond the three already-previewed early beats.
...
The next implementation slice remains `wuxia_seo_harin_rescue`, mapped from Notion DB row `wuxia_seoharin_intervention` / `서하린의 개입`.

No additional design is required before implementing that slice unless the desired scope expands beyond the current bridge: 서하린 intervention, outsider investigation, protection/watch, common `taken_under_watch`, `seo_harin_rescue_resolved`, and `cheongryu_outer_courtyard` hooks. The implementation must still avoid default office bundle changes, `escape-office` key changes, and new relation/debt/faction/reward schemas.
