...
- `wuxia_mumyeong_followup_after_orthodox_style_trace`에서 `wuxia_mumyeong_midgame_reunion`, `wuxia_mumyeong_departure_truth_summary`, `wuxia_boss_first_appearance`, `wuxia_qingliu_attack_after_war`를 비교한다.
- 다음 handoff는 runtime YAML/Rust/Web generated artifact를 수정하지 않고, 무명 재회/진실 reveal/boss wall/full flashback을 바로 열지 말지 먼저 결정한다.
