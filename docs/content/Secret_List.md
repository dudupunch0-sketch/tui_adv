...
혼자 출력된 종이의 빈 선택지와 회의실 화이트보드 모서리의 지워지지 않는 표식을 이어, 회의실 기록 밖에 남은 세 번째 안전한 현실 메모 후보를 암시한다.
...
- printer_prints_alone
- meeting_room_whiteboard_marker
...
1. 복합기 구역에서 혼자 출력된 종이를 읽어 `future_choice_printout`을 얻는다.
2. 회의실에서 화이트보드 모서리 표식을 확인해 `reality_link_hint_3`와 `reality_link_third_seen`을 얻는다.
3. 출력물 단서와 세 번째 표식 플래그가 모두 있으면 `hidden_reality_hint_003`이 열린다.
...
실제 라벨 또는 사무실 표식 정보는 `private/secrets.local.yaml`의 `actual_ip_address`에만 둔다.

안전 체크:

- 공용 회의실 또는 개발자가 설치한 메모만 사용한다.
- 화이트보드 내용, 회의 기록, 개인 메모를 공개 단서로 쓰지 않는다.
- 공개 저장소에는 최종 위치를 쓰지 않는다.

## Secret 004 후보: 사내망의 반대편
