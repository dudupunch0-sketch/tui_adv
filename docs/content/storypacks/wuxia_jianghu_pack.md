...
- `wuxia_mumyeong_followup_after_first_confrontation`에서 Notion 사건 카드 DB `wuxia_mumyeong_copy_style_reveal` / `무명의 카피 무공 공개`, `wuxia_mumyeong_midgame_reunion`, `wuxia_boss_first_appearance`를 비교한다.
- 카피 무공 공개가 필요하더라도 seed 기반 random copy-style system, 천외편린 reward schema, boss combat, route graph/faction reputation/debt/relation ledger는 먼저 문서에서 경계를 정한다.
