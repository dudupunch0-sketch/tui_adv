...
const server = createServer(async (req, res) => {
  ...
  const file = path.join(here, rel === '/' ? 'faces.html' : rel);
...
const shots = [];
try {
  const page = await browser.newPage({ viewport: { width: 1260, height: 1100 }, deviceScaleFactor: 3 });
  const errors = [];
  ...
  await page.goto(`http://127.0.0.1:${port}/faces.html`, { waitUntil: 'load' });
  ...
  const sheet = path.join(outDir, 'style1-faces.png');
