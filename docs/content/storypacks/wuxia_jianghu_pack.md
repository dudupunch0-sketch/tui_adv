...
- `wuxia_mumyeong_first_confrontation_after_sighting`에서 Notion 사건 카드 DB `wuxia_mumyeong_first_confrontation` / `무명 첫 대치`를 다음 runtime 후보로 열지 결정한다.
- 첫 대치가 필요하더라도 combat resolver/schema, route graph, faction reputation, relation/debt ledger는 먼저 문서에서 경계를 정한다.
