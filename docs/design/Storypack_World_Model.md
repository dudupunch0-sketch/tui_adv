...
- `docs/content/storypack_db/storypacks.json`에 office/wuxia storypack record를 둔다.
- `docs/content/storypack_db/encounter_situations.json`에 `isolation_pack` 6개와 `wuxia_jianghu_pack` 6개 후보 카드를 둔다.
...
- 기존 encounter schema로 무협 인카운터 1~2개를 넣는다.
- 첫 후보는 `wuxia_commute_rift_arrival` 또는 `wuxia_heuksa_bang_first_fight`다.
...
- 런타임 구현: 미착수.
- 다음 추천: `docs/dev/Storypack_Runtime_Preview_Mode.md`의 separate preview mode first 결정에 따라, 기존 encounter schema만 쓰는 `wuxia_commute_rift_arrival` 또는 `wuxia_heuksa_bang_first_fight` preview bundle prototype을 연다.
