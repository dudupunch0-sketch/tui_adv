...
Status: 결정 문서 + final epilogue runtime 구현 완료 + return/settlement branch consumer 구현 완료 + battle-loss branch consumer 구현 완료 + final-state canonical collapse runtime 구현 완료
...
Latest runtime output: `epilogue_state_audit` body block from Rust GameCore. The next contract decision is `wuxia_final_state_canonical_collapse_followup_handoff`.
