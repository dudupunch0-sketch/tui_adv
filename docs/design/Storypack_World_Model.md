...
- 다음 추천: `wuxia_mumyeong_followup_after_orthodox_style_trace` docs-only handoff에서 `wuxia_mumyeong_midgame_reunion`, `wuxia_mumyeong_departure_truth_summary`, `wuxia_boss_first_appearance`, `wuxia_qingliu_attack_after_war`를 비교한다. legacy office bundle/legacy `escape-office` key/seed 기반 random copy-style system/천외편린 3택 성장/return system/faction route graph/debt ledger/combat schema는 변경하지 않는다.
