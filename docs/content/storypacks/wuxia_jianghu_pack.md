...
28. `wuxia_sado_final_phase_2_weakpoint_control` — preview runtime 구현 완료
   ...
   - combat resolver, HP 숫자전, final epilogue/return schema, 서하린 truth delivery, `told_seoharin_truth`, 무명 구원 확정, relation/reward schema, `item_unpriced_wooden_sword` payout은 열지 않는다. 다음 handoff 후보는 `wuxia_sado_final_phase_3_outside_calculation`이다.
