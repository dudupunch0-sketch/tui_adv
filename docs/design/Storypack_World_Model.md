...
- `docs/content/storypack_db/encounter_situations.json`에 `isolation_pack` 6개, `yageunmong_pack` 6개, `wuxia_jianghu_pack` 6개 후보 카드를 둔다.
...
### Step 4 — first runtime wuxia route slice

진행 상태: `wuxia_commute_rift_arrival` preview는 완료했고, 다음 구현 slice는 `wuxia_heuksa_bang_first_fight`로 확정한다.

- `wuxia_heuksa_bang_first_fight`는 같은 `storypack_preview` bundle에만 추가한다.
- 기존 encounter schema로 무협 첫 전투를 넣는다: `conditions`, `choices`, `cost`, `outcome.resources`, `danger`, `add_flags`, `add_clues`, `add_items`/`remove_items`, `destination_id`, `log`, optional `presentation`.
- 안정 choice id는 `run_toward_open_street`(fallback), `deescalate_with_words`, `swing_commute_bag`, `loosen_tie_and_drop_shoes`, `crash_in_with_body`를 우선한다.
- `ScenePage` schema 변경 없이 진행한다.
- CLI/Rust/Web generated content smoke로 office가 아닌 location/encounter도 표시되는지 검증한다.
- preview launcher/UI wiring은 follow-up 후보이며 first fight의 선행 조건이 아니다.
...
- 상태: 설계 문서화 + machine-readable storypack DB 검증 + preview mode 결정 완료.
...
- 런타임 구현: `wuxia_commute_rift_arrival` preview는 완료, `yageunmong_pack` runtime은 미착수.
- 다음 추천: 무협은 기존 encounter schema만 쓰는 `wuxia_heuksa_bang_first_fight` preview 구현으로 확정한다. preview launcher/UI wiring은 이후 명시적 opt-in UX 후보로 두고, 야근몽은 `yageunmong_late_night_desk_awake` 또는 각성편린 3택 preview 후보로 둔다.
