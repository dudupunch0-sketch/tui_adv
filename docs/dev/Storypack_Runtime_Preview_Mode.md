...
Status: 결정 문서 + `wuxia_seoharin_left_meal` runtime 구현 완료
...
Next docs-only handoff: `wuxia_seoharin_left_meal_followup`. Compare `wuxia_seoharin_unsaid_stay`, `wuxia_mumyeong_resolution`, `wuxia_boss_resolution`, `wuxia_sado_final_battle`, final-route state dictionary, and remaining late/final candidates before opening any truth delivery, `told_seoharin_truth`, Mumyeong salvation confirmation, final resolution, epilogue/return schema, combat schema, route/faction/relation/debt/reward schema, or `item_unpriced_wooden_sword` payout.
