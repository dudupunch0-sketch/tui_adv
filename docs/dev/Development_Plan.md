...
>
> Rust 전투 개입의 다음 prerequisite 순서는 **I2b-S1a → I2b-S1b → I2b-S2 → I2b-S3**다. S1a는 occurrence target/observer-combatant carrier, full static roster `CombatOpportunityContext.combatant_ids: Vec<String>`, self-contained fingerprint carrier `CombatOpportunityEvaluation.context_combatant_ids: Vec<String>`을 소유한다(`combat_opportunity.rs`, wave1 tests, repo 전체 mandatory context/evaluation literal compile fix). Context Vec는 duplicate ingress evidence를 보존하며 기존 `evaluate(instances, context)` signature를 넓히지 않고 local validation이 empty·blank·duplicate와 observer mapping nonmembership을 canonicalization 전에 reject한다. Evaluate만 evaluation roster를 validated stable-sorted form으로 채우고 canonical JSON/no-argument fingerprint가 이를 포함한다. S1a에서 checkpoint/save/lifecycle/transaction/content authoring과 fingerprint method argument 추가는 금지한다. Actual request roster equality는 S1b의 `CombatRuntime::with_opportunities`가 evaluation 전에 검사하고, restore는 evaluation/context/request three-way equality와 canonical form을 fingerprint 재계산/restore acceptance 전에 검사한다. Runtime constructors는 request participants에서 canonical sorted Vec를 직접 derive하고 active/hp 판단도 S1b snapshot owner다. S1b는 `combat_runtime.rs`와 focused tests만 소유해 이 roster gate, trigger-tick 시작 health/active snapshot, stable pause ID, selected-target exact-one/no-fallback preflight, checkpoint v2와 provenance-less paused v1 reject를 구현하며 `save.rs`, `lib.rs`, GameState/terminal/transaction/authoring schema는 금지한다. 각 slice는 merge와 독립 review 후 다음 slice로 진행하며 exact acceptance/stop 조건은 [combat contract handoff](../content/design_source/handoffs/combat_contract_handoff.md)의 “WP-I2b-S1 승인 정본”과 “별도 work package와 acceptance”를 따른다. `SaveEnvelope` v2는 I7b까지 금지한다.
>
...
- (UI 트랙) **ACTIVE Web renderer track: Three.js WP1 pure adapter contract.** `web/src/ui/storybook/combat/combatThreeAdapter.ts`와 단위 테스트, Rust producer가 만든 canonical combat fixture, Rust byte golden test만 구현한다. 이 트랙은 Rust I2b/I7과 다른 game/content track에 독립적이므로 병렬 진행할 수 있으며 그 트랙들의 우선순위를 대체하지 않는다. 기존 `renderCombatStage` DOM이 visible surface로 남고 WP2 canvas/mount는 승인되지 않았다. 구현자는 `docs/design/ThreeJS_Combat_Visual_Architecture.md` §3~4와 §10.1~11.1을 먼저 읽고, `docs/design/Combat_System_Implementation_Plan_Index.md`의 ACTIVE WP1 항목을 확인한다. WP1 acceptance 뒤 멈추고 review를 기다린다.
