...
def test_wuxia_baekdo_medicine_debt_runtime_slice_is_docs_synced():
    ...
    assert "- `wuxia_jianghu_pack`: 이구학지 — 천기록 후보 카드 31개." in storypack_db_readme
...
def test_wuxia_mumyeong_destroys_orthodox_sect_runtime_slice_is_docs_synced():
    ...
    assert "서른한 개 implemented preview encounter" in world_model
    assert "현악문 consequence trace도 기존 encounter schema로 구현 완료" in encounter_model
    assert "Latest implemented runtime: `wuxia_mumyeong_resolution`" in runtime_preview
    assert "Next design/runtime gate: `wuxia_seoharin_qingliu_resolution` handoff" in runtime_preview
    assert "runtime_status: \"implemented_in_storypack_preview\"" in notion_sources
...
def test_wuxia_boss_recruits_mumyeong_runtime_slice_is_docs_synced():
    ...
    assert "Latest runtime slice: wuxia_mumyeong_resolution" in storypack_db_json
...
def test_wuxia_departure_truth_summary_followup_selects_seoharin_empty_place():
    ...
    assert "current_goal: wuxia_seoharin_qingliu_resolution_handoff" in next_goal
    assert "previous_current_goal: wuxia_mumyeong_resolution" in next_goal
    ...
    assert "- `wuxia_jianghu_pack`: 이구학지 — 천기록 후보 카드 31개." in storypack_db_readme
    assert "Latest runtime slice: wuxia_mumyeong_resolution" in storypack_db_json
    assert "Latest handoff: wuxia_mumyeong_resolution_handoff implemented Mumyeong resolution route seed bridge and deferred wuxia_seoharin_qingliu_resolution" in storypack_db_json
    assert "Next runtime candidate: wuxia_seoharin_qingliu_resolution" in storypack_db_json
    ...
    assert "Latest implemented runtime: `wuxia_mumyeong_resolution`" in runtime_preview
    assert "Next design/runtime gate: `wuxia_seoharin_qingliu_resolution` handoff" in runtime_preview
    assert "`wuxia_mumyeong_resolution`까지 YAML/generated bundle에 반영" in world_model
...
def test_wuxia_final_state_routing_contract_is_docs_synced():
    ...
    assert "Next design/runtime gate: `wuxia_seoharin_qingliu_resolution` handoff" in runtime_preview
    assert "Latest implemented runtime is `wuxia_mumyeong_resolution`" in runtime_preview
...
def test_wuxia_sado_final_phase_1_price_tag_runtime_slice_is_docs_synced():
    ...
    assert "Next design/runtime gate: `wuxia_seoharin_qingliu_resolution` handoff" in runtime_preview
    assert "`wuxia_mumyeong_resolution`까지 YAML/generated bundle에 반영" in world_model
    assert "`wuxia_mumyeong_resolution`도 기존 encounter schema로 구현 완료" in encounter_model
    assert "told_seoharin_truth" not in encounters_yaml
