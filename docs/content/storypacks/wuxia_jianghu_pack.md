...
25. `wuxia_seoharin_empty_place` — 다음 runtime 후보
   - `wuxia_mumyeong_departure_truth_summary_followup` handoff에서 다음 runtime 후보로 결정했다.
   - Notion 원본의 초반 companion beat를 sealed truth 이후 late emotional bridge로 재해석해, 서하린의 기다림을 소유가 아니라 돌아올 자리로 이해하는 hook을 만든다.
   - 다음 구현은 `seoharin_axis_opened`, `empty_place_remembered`, `unpriced_wooden_sword_condition_seeded` clue/flag 후보를 남기되, `told_seoharin_truth`, 무명 구원 확정, 사도 최종전, final/epilogue/return schema는 열지 않는다.
