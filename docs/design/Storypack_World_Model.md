...
- 상태: 설계 문서화 + machine-readable storypack DB 검증 + preview mode 결정 + 무협 열아홉 개 preview runtime + `wuxia_mumyeong_request_for_aid` 구현 완료 + Web/terminal default 이구학지 wiring 완료.
...
- 다음 추천: `wuxia_mumyeong_followup_after_failed_aid` docs-only handoff. legacy office bundle/legacy `escape-office` key/seed 기반 random copy-style system/천외편린 3택 성장/return system/faction route graph/debt ledger/combat schema/boss combat/final boss resolution은 변경하지 않는다.
