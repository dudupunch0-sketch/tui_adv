...
1. 무협 storypack preview/main의 다음 작업은 `wuxia_mumyeong_followup_after_midgame_reunion` docs-only handoff다. `wuxia_mumyeong_midgame_reunion` runtime implementation은 완료됐고, 다음 세션은 후속 후보를 Notion 사건 카드 DB와 repo runtime hook에 맞춰 비교한다.
   ...
   - 다음 handoff 후보는 `wuxia_mumyeong_departure_truth_summary`, `wuxia_boss_first_appearance`, `wuxia_qingliu_attack_after_war`를 우선 비교한다.
   ...
   - 보스 첫 등장, 무명 이탈 진실 정리, 청류문 습격 전체 과거 회상은 보류한다. 후속 docs-only handoff에서도 truth reveal, 구원 확정, 서하린에게 진실 전달, boss combat/final-wall pressure, full flashback을 바로 열지 말지 먼저 문서에서 결정한다.
...
1. 다음 무협 storypack preview/main 작업은 `wuxia_mumyeong_followup_after_midgame_reunion` docs-only handoff다.
   ...
   - 다음 handoff는 `wuxia_mumyeong_departure_truth_summary`, `wuxia_boss_first_appearance`, `wuxia_qingliu_attack_after_war`를 우선 비교한다.
   - seed 기반 random copy-style system/table, 천외편린 3택 reward/ability schema, boss combat/final-wall pressure, 무명 과거 진실 전체 reveal, 서하린에게 진실 전달은 바로 열지 않는다.
