...
Status: docs-only sync + Mumyeong awakening runtime implemented + Qingliu attack trace next runtime selected
Last checked: 2026-06-02T08:44:45+09:00
Runtime status: `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt`, `wuxia_black_heaven_escape_price`, `wuxia_heavenly_archive_previous_outsiders`, `wuxia_wounded_shelter_dawn_offers`, `wuxia_mumyeong_first_sighting`, `wuxia_mumyeong_first_confrontation`, `wuxia_mumyeong_copy_style_reveal`, `wuxia_mumyeong_reads_orthodox_style`, `wuxia_mumyeong_midgame_reunion`, `wuxia_boss_first_appearance`, `wuxia_mumyeong_request_for_aid`, and `wuxia_mumyeong_awakening` were implemented later as separate `storypack_preview` runtime slices. The next repo step is `wuxia_qingliu_attack_after_war` runtime implementation as a restricted attack-trace investigation. This document remains the Notion coverage ledger. 2026-06-01 default 전환 이후 Web player 기본 storypack은 이구학지이며, 2026-06-02 terminal content scene default도 이구학지 fixture로 맞췄다.
...
| 17 | `wuxia_qingliu_attack_after_war` | 무너져가는 청류문 습격 | `wuxia_qingliu_attack_after_war` | next runtime selected (limited trace investigation) |
...
- The selected `wuxia_qingliu_attack_after_war` follow-up is not runtime implemented yet; it is only selected as the next restricted attack-trace investigation.
