...
1. 무협 storypack preview/main의 다음 작업은 `wuxia_mumyeong_resolution` handoff/runtime candidate 검토다. `wuxia_sado_final_phase_1_price_tag`, `wuxia_sado_final_phase_2_weakpoint_control`, `wuxia_sado_final_phase_3_outside_calculation`, `wuxia_boss_resolution`은 기존 encounter schema로 구현 완료됐고, `docs/design/Wuxia_Final_State_Routing.md`가 canonical final inputs/result priority/alias policy를 소유한다.
...
1. 다음 무협 storypack preview/main 작업은 `wuxia_mumyeong_resolution` handoff/runtime candidate 검토다. `wuxia_sado_final_phase_1_price_tag`, `wuxia_sado_final_phase_2_weakpoint_control`, `wuxia_sado_final_phase_3_outside_calculation`, `wuxia_boss_resolution`은 final state routing contract 기준으로 구현 완료했다.
   ...
   - stable choice id는 `eat_the_left_meal_quietly`, `thank_seoharin_for_the_bowl`, `joke_about_who_ordered_extra_rice`, `pass_without_eating_the_meal`다.
   - common hook은 `seoharin_left_meal_resolved`, `truth_delivery_still_unopened`, `destination_id: cheongryu_outer_courtyard`이며, 긍정 선택은 `seoharin_axis_deepened`/`qingliu_belonging_warmed`, 거절 선택은 `seoharin_axis_still_open`/`left_meal_left_untouched`를 남긴다.
   - `wuxia_sado_final_phase_1_price_tag`, `wuxia_sado_final_phase_2_weakpoint_control`, `wuxia_sado_final_phase_3_outside_calculation`, `wuxia_boss_resolution`는 preview runtime 구현 완료다. ledger/evidence/pressure/item-log seed, weakpoint/final-method seed, outside-calculation result candidate seed, boss-resolution route seed를 기존 encounter schema로만 남겼고, 다음 runtime 후보는 `wuxia_mumyeong_resolution` handoff다. `wuxia_sado_final_battle`, `wuxia_seoharin_unsaid_stay`, 남은 final/late companion 후보는 계속 보류한다.
