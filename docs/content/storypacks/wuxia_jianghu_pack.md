...
runtime_promotion_notes: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt`, `wuxia_black_heaven_escape_price`, `wuxia_heavenly_archive_previous_outsiders`, `wuxia_wounded_shelter_dawn_offers`, `wuxia_mumyeong_first_sighting` runtime은 separate storypack preview bundle에서 완료했다. `wuxia_mumyeong_first_confrontation_after_sighting` docs-only handoff는 다음 runtime 후보를 `wuxia_mumyeong_first_confrontation`로 결정했다. 2026-06-01 default 전환 이후 Web player 기본 경로는 이구학지이며, office content는 legacy/parity fixture로 남긴다. 기본 office bundle, legacy `escape-office` save/localStorage key, 천외편린 3택 성장 schema, faction/relation/debt ledgers는 아직 열지 않는다.
```
...
| `wuxia_mumyeong_first_confrontation` | `midgame_rival` / `rival_confrontation` | `sect_courtyard`, `training_chore`, `faction_negotiation` | 첫 목격 이후 무명을 라이벌로 확정하는 첫 대치다. 이기는 전투가 아니라 버티기/관찰/카피 무공 분석/서하린 침묵 확인으로 구현한다. | 다음 runtime 후보 |
...
14. `wuxia_mumyeong_first_confrontation` — docs-only handoff ready
   - `wuxia_mumyeong_first_confrontation_after_sighting` handoff에서 다음 runtime 후보로 결정했다.
   ...
   - stable choice id 후보는 `meet_mumyeong_head_on`, `endure_until_copy_flow_breaks`, `watch_seo_harin_hold_back`, `read_mumyeongs_copied_form`, `do_not_provoke_mumyeong`다.
...
- `wuxia_mumyeong_first_confrontation`를 같은 storypack preview source에 추가한다.
- 첫 runtime pass는 `mumyeong_first_confrontation_resolved`, `mumyeong_rival_thread_opened`, `copied_flow_weakness_noted`, `seo_harin_mumyeong_silence_confirmed` 같은 flags/clues/log/presentation만 남긴다.

주의:

- 무협 콘텐츠는 office legacy content와 섞지 않고 `wuxia_jianghu_pack` bundle 경로로 연다. terminal fixture는 `storypack_preview` metadata를 유지하고, Web default는 이 bundle을 이구학지 main run으로 사용한다.
