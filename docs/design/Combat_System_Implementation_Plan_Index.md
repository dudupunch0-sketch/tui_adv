...
- **전투를 시작하는 인카운터 authoring — `ScenePage.combat`의 producer가 없어 현재 항상 `None` → Wave 3 Step 2.** encounter schema에 전투를 여는 필드도, 전투 stage kind도 아직 없다. Step 2 authoring이 없으면 Step 1d 렌더러가 표시할 데이터가 없으므로, 단계 순서를 `1c → Step 2 → 1d`로 조정했다 (아래 "단계 순서" 표).
