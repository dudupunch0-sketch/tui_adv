...
- 현재 기본 개발 기준팩: `wuxia_jianghu_pack` / **이구학지 — 천기록**. 전제는 “현대 회사원이 본인 몸과 출근복장 그대로 무협 세계 시장에 전이되고, 천기록/천외편린 성장 구조를 경험한다”. 현재 Web/terminal default storypack이며, runtime은 arrival/first fight/first fragment부터 `wuxia_mumyeong_departure_truth_summary`까지 구현됐다. 다음은 `wuxia_mumyeong_departure_truth_summary_followup` docs-only handoff에서 무명/보스/서하린 late truth 후보를 비교한다.
- 1차 재난 타입: 불명 재난
