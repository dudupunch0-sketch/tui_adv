...
Observer identity와 combatant identity는 분리한다. Public `CombatObserver`에 explicit `combatant_id`를 추가하고, public `CombatOpportunityContext`에 `combatant_ids: BTreeSet<String>`을 추가한다. `combatant_ids`는 해당 actual combat request의 pause/static participant roster 전체 ID set과 exact-equal해야 하며 active/hp 후보 set이 아니다. Raw/list authoring 또는 runtime ingress는 blank/whitespace-only와 duplicate를 **BTreeSet 구성 전에** reject해서 duplicate 증거를 잃지 않는다. 이미 typed `BTreeSet`을 받는 context validation은 non-empty, 각 member nonblank, combat request participant roster와 exact set equality를 다시 검증한다. `observer.id`는 detection/opportunity provenance ID이고 executor/target ID가 아니며, 두 문자열이 우연히 같아도 equality를 mapping으로 사용하지 않는다. Opportunity context validation은 observer ID와 mapped combatant ID의 blank, `combatant_ids` non-membership, duplicate observer ID, 둘 이상의 observer가 같은 combatant로 매핑되는 경우를 reject한다. `observer` executor는 exact opportunity observer의 mapped combatant다. `any_capable`은 observer의 `can_act`와 required capabilities, mapped combatant의 pause-active와 hp>0을 모두 만족하는 후보만 남기고 mapped combatant ID 오름차순으로 exactly one을 고른다. Capability는 observer record, static membership은 context roster, active/health는 S1b의 combat runtime snapshot이 owner다. 새 entity registry나 authoring identity 의미는 만들지 않는다.

전체 roster는 `CombatOpportunityCandidate`에 복제하지 않는다. Evaluation/pause fingerprint canonical input은 evaluation payload 뒤에 `CombatOpportunityContext.combatant_ids`의 stable ID 오름차순 배열을 별도 항목으로 포함한다. 따라서 roster source의 입력 순서 차이는 fingerprint에 영향이 없고, 같은 candidate가 선택되더라도 roster membership이 하나라도 달라지면 fingerprint는 달라진다. Candidate canonical JSON만으로 roster-aware fingerprint를 대신해서는 안 된다.
...
1. **I2b-S1a — occurrence/identity carrier**: 선행 조건은 PR #219의 I2 prerequisite authoring contract merge다. Owned files는 `crates/escape-core/src/combat_opportunity.rs`, `crates/escape-core/tests/combat_opportunity_wave1.rs`, 그리고 mandatory field 추가로 깨지는 struct literal만 고치는 `crates/escape-core/src/combat_runtime.rs`의 compile fixture/test 영역이다. `CombatOpportunityInstance.bound_target_ids`/`bound_target_tick`, candidate exact copy/fingerprint, `CombatObserver.combatant_id`, `CombatOpportunityContext.combatant_ids`, stable mapped-combatant selection을 구현한다. Repo 전체의 기존 `CombatOpportunityContext` struct literal compile fix는 S1a owner가 함께 수행하되 roster field 주입 외 behavior 변경은 금지한다. Acceptance는 raw roster blank/duplicate pre-set reject, typed roster empty/blank/exact-equality reject, observer mapped ID membership reject, roster input-order fingerprint parity와 membership-change fingerprint delta, bound vector order-independent canonical fingerprint, bound value/tick change fingerprint delta, observer vector reversal parity, capability/can_act filtering이다. Stop은 content authoring schema 변경, participant/entity registry 생성, runtime checkpoint/lifecycle/transaction mutation이 필요할 때다. Forbidden files는 `save.rs`, `combat_intervention_transaction.rs`, TS/Web/generated/storypack, `docs/content/design_source/schema/combat_intervention_response.schema.json`이다.
