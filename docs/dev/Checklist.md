...
- [ ] `pyproject.toml` 생성
- [ ] `src/tui_adv/__init__.py` 생성
- [ ] `src/tui_adv/main.py` 생성
- [ ] `src/tui_adv/game/` 패키지 생성
- [ ] `src/tui_adv/tui/` 패키지 생성
- [ ] `src/tui_adv/data/` 디렉터리 생성
- [ ] `tests/` 디렉터리 생성
...
- [ ] 기본 실행 명령 정의
- [ ] 앱이 “준비 중” 메시지라도 출력하며 실행된다.
- [ ] `pytest`가 실행된다.
- [ ] README에 초기 실행 방법을 기록한다.
...
- [ ] 체력 0 실패 상태 정의
- [ ] 정신력 0 실패 상태 정의
- [ ] 허기 한계 실패 또는 지속 피해 정의
- [ ] 갈증 한계 실패 또는 지속 피해 정의
- [ ] 배터리 0일 때 제한되는 행동 정의
- [ ] 정신력 낮음 선택지 왜곡 규칙 초안 구현 또는 문서화
- [ ] 갈증 높음 환각 이벤트 규칙 초안 구현 또는 문서화
