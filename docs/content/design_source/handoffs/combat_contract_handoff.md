...
전체 roster는 `CombatOpportunityCandidate`에 복제하지 않는다. S1a의 evaluation/pause fingerprint canonical input은 local validation 뒤 만든 stable-ID-sorted clone을 사용하고, S1b의 actual roster equality gate도 같은 canonicalization 규칙을 사용한다. Fingerprint는 evaluation payload 뒤에 이 clone을 별도 항목으로 포함한다. 따라서 unique roster 입력 순서 차이는 fingerprint에 영향이 없고, 같은 candidate가 선택되더라도 roster membership이 하나라도 달라지면 fingerprint는 달라지며, duplicate 입력은 fingerprint 단계까지 도달하지 못한다. Candidate canonical JSON만으로 roster-aware fingerprint를 대신해서는 안 된다.

Actual roster equality gate는 S1b의 `CombatRuntime`이 소유한다. `CombatRuntime::with_opportunities(request, config)`는 request participant IDs에서 canonical sorted Vec를 직접 derive할 수 있는 constructor path에서는 그 값을 context에 공급하고, caller-provided context를 받는 path에서는 validated sorted context clone과 request participant ID set의 exact equality를 **catalog evaluation 전에** 검사한다. Restore는 checkpoint의 `CombatResolutionRequest`와 restored opportunity context를 같은 규칙으로 비교한 뒤에만 runtime을 받아들이며 mismatch는 evaluation/restore state acceptance 전에 `InvalidInput`으로 reject한다. 이 gate는 active/hp filtering과 별개다.
