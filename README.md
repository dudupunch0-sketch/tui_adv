...
- 현재 기본 개발 기준팩: `wuxia_jianghu_pack` / **이구학지 — 천기록**. 전제는 “현대 회사원이 본인 몸과 출근복장 그대로 무협 세계 시장에 전이되고, 천기록/천외편린 성장 구조를 경험한다”. 현재 Web/terminal default storypack이며, runtime은 arrival/first fight/first fragment부터 `wuxia_boss_recruits_mumyeong`까지 구현됐다. `wuxia_boss_recruits_mumyeong_followup` docs-only handoff에서 다음 runtime 후보를 `wuxia_mumyeong_departure_truth_summary`로 선택했고, 구현은 sealed summary로 제한한다.
- 1차 재난 타입: 불명 재난
...
- `src/tui_adv/storypack-previews/wuxia_jianghu_pack/`: `wuxia_commute_rift_arrival`부터 `wuxia_mumyeong_destroys_orthodox_sect`까지의 이구학지 source YAML
