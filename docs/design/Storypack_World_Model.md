...
현재 Web/default storypack은 `wuxia_jianghu_pack` / **이구학지 — 천기록**이다. `escape from the office` office isolation content는 legacy/parity 기준팩으로 유지한다. 앞으로의 개발 기준은 다음처럼 둔다.
...
- `docs/content/storypack_db/encounter_situations.json`에 `isolation_pack` 6개, `yageunmong_pack` 6개, `wuxia_jianghu_pack` 15개, 총 27개 repo 후보 카드를 둔다. Notion 사건 카드 DB의 26개 row는 `docs/dev/Notion_Design_Coverage.md`에서 매핑한 upstream source이며, machine-readable mirror에 전부 자동 import하지 않는다.
...
- `wuxia_mumyeong_reads_orthodox_style`는 다음 runtime 후보이며, 안정 choice id 후보는 `compare_copied_form_to_old_wound`, `trace_qingliu_eye_variation`, `reconstruct_mumyeongs_sightline`, `stop_before_truth_becomes_accusation`다.
...
- 상태: 설계 문서화 + machine-readable storypack DB 검증 + preview mode 결정 + 무협 열다섯 개 preview runtime + Web default 이구학지 wiring 완료.
...
- 런타임 구현: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `wuxia_baekdo_medicine_debt`, `wuxia_black_heaven_escape_price`, `wuxia_heavenly_archive_previous_outsiders`, `wuxia_wounded_shelter_dawn_offers`, `wuxia_mumyeong_first_sighting`, `wuxia_mumyeong_first_confrontation`, `wuxia_mumyeong_copy_style_reveal` preview는 완료했다. `wuxia_mumyeong_first_sighting`는 `route_midgame_continuity_after_wounded_shelter` handoff 결과로 구현됐고, `wuxia_mumyeong_first_confrontation`는 `wuxia_mumyeong_first_confrontation_after_sighting` handoff 결과로 구현됐다. `wuxia_mumyeong_copy_style_reveal`는 `wuxia_mumyeong_followup_after_first_confrontation` handoff 결과로 구현됐다. `wuxia_mumyeong_followup_after_copy_style_reveal` handoff는 `wuxia_mumyeong_reads_orthodox_style`를 다음 runtime 후보로 골랐다. `yageunmong_pack` runtime은 미착수다.
- 다음 추천: `wuxia_mumyeong_reads_orthodox_style` runtime implementation에서 현악문/복호금쇄수 clue를 기존 flags/clues/log/presentation으로 먼저 남긴다. legacy office bundle/legacy `escape-office` key/seed 기반 random copy-style system/천외편린 3택 성장/return system/faction route graph/debt ledger/combat schema는 변경하지 않는다.
