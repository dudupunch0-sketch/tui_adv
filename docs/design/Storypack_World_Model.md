...
- 상태: 설계 문서화 + machine-readable storypack DB 검증 + preview mode 결정 + 무협 열여덟 개 preview runtime + `wuxia_boss_followup_after_first_appearance` handoff 대기 + Web/terminal default 이구학지 wiring 완료.
...
- 다음 추천: `wuxia_boss_followup_after_first_appearance` docs-only handoff. legacy office bundle/legacy `escape-office` key/seed 기반 random copy-style system/천외편린 3택 성장/return system/faction route graph/debt ledger/combat schema/boss combat/final boss resolution은 변경하지 않는다.
