...
- 상태: 설계 문서화 + machine-readable storypack DB 검증 + preview mode 결정 + 무협 스무 개 preview runtime + 21번째 후보 `wuxia_qingliu_attack_after_war` 선택 + Web/terminal default 이구학지 wiring 완료.
...
- 다음 추천: `wuxia_qingliu_attack_after_war` runtime implementation. full Qingliu attack flashback이 아니라 현악문/복호금쇄수 흔적 조사로 구현하고, legacy office bundle/legacy `escape-office` key/seed 기반 random copy-style system/천외편린 3택 성장/return system/faction route graph/debt ledger/combat schema/boss combat/final boss resolution은 변경하지 않는다.
