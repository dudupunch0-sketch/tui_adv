...
진행 상태: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment` preview는 완료했다. 다음 구현 후보는 `wuxia_seo_harin_rescue` 또는 `wuxia_cheongryu_apprentice_entry` bridge다.

- 세 preview encounter는 같은 `storypack_preview` bundle에만 둔다.
- 기존 encounter schema로 무협 route slice를 넣는다: `conditions`, `choices`, `cost`, `outcome.resources`, `danger`, `add_flags`, `add_clues`, `add_items`/`remove_items`, `destination_id`, `log`, optional `presentation`.
