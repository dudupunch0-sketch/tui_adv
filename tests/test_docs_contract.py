...
def test_wuxia_cheongryu_raid_wounded_fallback_runtime_slice_is_docs_synced():
    ...
    assert "current_goal: wuxia_mumyeong_destroys_orthodox_sect" in next_goal
...
def test_wuxia_baekdo_medicine_debt_runtime_slice_is_docs_synced():
    ...
    assert "- `wuxia_jianghu_pack`: 이구학지 — 천기록 후보 카드 22개." in storypack_db_readme
...
def test_wuxia_mumyeong_request_for_aid_runtime_slice_is_docs_synced():
    ...
    assert "wuxia_mumyeong_destroys_orthodox_sect" in next_goal
    assert "wuxia_mumyeong_followup_after_awakening" in next_goal
    assert "wuxia_mumyeong_request_for_aid" in world_model
...
def test_wuxia_mumyeong_awakening_runtime_slice_is_docs_synced():
    ...
    assert "wuxia_qingliu_attack_after_war" in next_goal
    assert "inspect_bokho_lock_scars" in next_goal
    assert "compare_hyeonakmun_trace_to_qingliu_wounds" in next_goal
    assert "ask_seo_harin_what_she_saw_afterward" in next_goal
    assert "stop_before_replaying_the_attack" in next_goal
    assert "wuxia_mumyeong_destroys_orthodox_sect" in next_goal
    assert "wuxia_boss_recruits_mumyeong" in next_goal
    assert "wuxia_mumyeong_followup_after_awakening" in next_goal
    assert "- `wuxia_jianghu_pack`: 이구학지 — 천기록 후보 카드 22개." in storypack_db_readme
...
def test_wuxia_qingliu_attack_after_war_handoff_is_docs_synced():
    ...
    assert "mumyeong_awakening_resolved" in next_goal
    assert "qingliu_attack_after_war_resolved" in next_goal
    assert "read_hyeonakmun_empty_gate_record" in next_goal
    assert "trace_bokho_lock_to_mumyeong" in next_goal
    assert "ask_why_seoharin_never_heard_full_story" in next_goal
    assert "stop_before_counting_the_dead" in next_goal
    assert "현악문 멸문 전투를 playable combat으로 만들기" in next_goal
    assert "- `wuxia_jianghu_pack`: 이구학지 — 천기록 후보 카드 22개." in storypack_db_readme
    ...
    assert "Latest implemented runtime: `wuxia_qingliu_attack_after_war`" in runtime_preview
    assert "runtime_status: \"implemented_in_storypack_preview\"" in notion_sources


def test_wuxia_mumyeong_destroys_orthodox_sect_handoff_is_docs_synced():
    plan = Path("docs/dev/Development_Plan.md").read_text(encoding="utf-8")
    checklist = Path("docs/dev/Checklist.md").read_text(encoding="utf-8")
    coverage = Path("docs/dev/Notion_Design_Coverage.md").read_text(encoding="utf-8")
    next_goal = Path("idea_box/next_goal/README.md").read_text(encoding="utf-8")
    ...
    assert (
        ...
        "`wuxia_mumyeong_destroys_orthodox_sect` | next runtime selected as limited consequence trace |"
    ) in coverage
    assert "current_goal: wuxia_mumyeong_destroys_orthodox_sect" in next_goal
    assert "mode: implementation" in next_goal
    assert "`wuxia_qingliu_attack_after_war_followup` docs-only handoff는 완료" in next_goal
    assert "read_hyeonakmun_empty_gate_record" in next_goal
    assert "trace_bokho_lock_to_mumyeong" in next_goal
    assert "ask_why_seoharin_never_heard_full_story" in next_goal
    assert "stop_before_counting_the_dead" in next_goal
    assert "- `wuxia_jianghu_pack`: 이구학지 — 천기록 후보 카드 22개." in storypack_db_readme
    assert "`wuxia_mumyeong_destroys_orthodox_sect` limited consequence trace" in storypack_db_readme
    assert "Next runtime candidate: wuxia_mumyeong_destroys_orthodox_sect" in storypack_db_json
    ...
    assert '"generated_artifacts_changed": false' in situations_json
    assert "| `wuxia_mumyeong_destroys_orthodox_sect` | `midgame_backstory` / `hyeonakmun_consequence_trace`" in wuxia_pack
    assert "22. `wuxia_mumyeong_destroys_orthodox_sect` — 다음 runtime 후보" in wuxia_pack
    ...
    assert "generated_artifacts_changed: false" in wuxia_cards
    assert "next_step: runtime_implementation" in wuxia_cards
    assert "limited consequence trace runtime implementation" in world_model
    assert "limited consequence trace" in encounter_model
    assert "Next runtime candidate: `wuxia_mumyeong_destroys_orthodox_sect`" in runtime_preview
