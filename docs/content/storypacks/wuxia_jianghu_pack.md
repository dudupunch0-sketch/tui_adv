...
11. `route_opener_followup_after_black_heaven` — 다음 docs-only handoff
   - 천기·귀환 opener 또는 `stabilize_wounded_until_dawn` branch를 받는 deferred-offer card 중 하나를 고른다.
   - 천기록 정체 reveal, 귀환 시스템, reward/ability schema, route graph/faction reputation/debt ledger는 열지 않는다.
