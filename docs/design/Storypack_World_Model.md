...
- `docs/content/storypack_db/encounter_situations.json`에 `isolation_pack` 6개, `yageunmong_pack` 6개, `wuxia_jianghu_pack` 23개, 총 35개 repo 후보 카드를 둔다. Notion 사건 카드 DB의 26개 row는 `docs/dev/Notion_Design_Coverage.md`에서 매핑한 upstream source이며, machine-readable mirror에 전부 자동 import하지 않는다.
...
- 상태: 설계 문서화 + machine-readable storypack DB 검증 + preview mode 결정 + 무협 스물세 개 preview runtime + Web/terminal default 이구학지 wiring 완료.
...
- 다음 추천: `wuxia_mumyeong_departure_truth_summary` runtime implementation. 구현은 `sealed_departure_truth_summary` trace로 제한하고, 서하린 truth delivery, `told_seoharin_truth`, 무명 구원 확정, 무명/보스 결산, epilogue/return, legacy office bundle/legacy `escape-office` key/seed 기반 random copy-style system/천외편린 3택 성장/faction route graph/debt ledger/combat schema/boss combat/final boss resolution은 바로 변경하지 않는다.
