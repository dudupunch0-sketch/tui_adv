...
- 현재 기본 개발 기준팩: `wuxia_jianghu_pack` / **이구학지 — 천기록**. 전제는 “현대 회사원이 본인 몸과 출근복장 그대로 무협 세계 시장에 전이되고, 천기록/천외편린 성장 구조를 경험한다”. 현재 Web/terminal default storypack이며, runtime은 arrival/first fight/first fragment부터 `wuxia_final_epilogue_renderer_contract`까지 구현됐다. 사도 최종전 1/2/3페이즈는 combat resolver 없이 장부/약점/계산식 밖 선택을 final-state seed로 남기고, 보스/무명/서하린·청류문/천기록 결산과 흑사방 후일담은 route/후속 epilogue candidate seed를 남긴다. `wuxia_final_epilogue_renderer_contract`는 이 seed를 Rust GameCore-owned structured `ScenePage.body_blocks`로 소비해 후일담 결과/카드/억제 기록을 출력한다. return/settlement, HP 숫자전, relation/debt/faction ledger, reward/ability, `told_seoharin_truth`, 천기록 정체 reveal은 아직 열지 않았다.
...
- `src/tui_adv/storypack-previews/wuxia_jianghu_pack/`: `wuxia_commute_rift_arrival`부터 `wuxia_final_epilogue_renderer_contract`까지의 이구학지 source YAML
