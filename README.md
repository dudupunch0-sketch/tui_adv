...
현재는 Phase 4 인카운터/선택지 도메인 엔진의 1차 골격까지 구현된 단계다.
게임 구조와 안전한 현실 연결 원칙을 문서화했고, 순수 게임 상태 모델, 자원 임계치/실패 판정, 1차 사무실 위치 모델, 인접 위치 이동, 인카운터/선택지 조건·비용·결과 적용, CLI smoke entry point를 추가했다.
...
2. 성공/실패 또는 확률 분기 모델 추가
3. 현재 위치에서 가능한 인카운터 선택 함수 추가
4. CLI에서 한 턴 인카운터 smoke 실행 지원
5. YAML 데이터 파일 분리 준비
