...
9. 여러 히든 현실 보물
10. Combat implementation is now tracked by docs/design/Combat_System_Implementation_Plan_Index.md; Wave 1 Step 1~3 and Wave 2 Step 1~3 (fixed integer positions, role/target contract, simultaneous tick frame, execution mode parity and dual logs, data-driven collision/attack/damage/effect resolution) are implemented and verified. Execute one plan file per coding subagent slice; multi-agent behavior/end conditions, renderer adapter, and balance remain split by approval.
