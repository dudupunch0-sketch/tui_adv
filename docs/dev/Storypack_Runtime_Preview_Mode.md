...
Status: 결정 문서 + `wuxia_mumyeong_departure_truth_summary` runtime 구현 완료 + next docs-only handoff selected
...
Next docs-only handoff: `wuxia_mumyeong_departure_truth_summary_followup`. Compare `wuxia_mumyeong_resolution`, `wuxia_boss_resolution`, `wuxia_seoharin_empty_place`, and remaining late truth/final candidates before opening any truth delivery, `told_seoharin_truth`, Mumyeong salvation confirmation, final resolution, epilogue/return schema, or route/faction/relation/debt/reward schema.
