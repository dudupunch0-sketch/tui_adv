...
| `wuxia_mumyeong_reads_orthodox_style` | `midgame_rival` / `orthodox_style_trace` | `sect_courtyard`, `cheonggi_record`, `training_chore` | 카피 무공 공개 뒤 무명이 과거에 읽어낸 정파식 제압술 흔적을 현악문/복호금쇄수 단서로 연결한다. | 다음 runtime 후보 |
...
16. `wuxia_mumyeong_reads_orthodox_style` — docs-only handoff 완료 / 다음 runtime 후보
   - `wuxia_mumyeong_followup_after_copy_style_reveal` handoff에서 다음 runtime 후보로 결정했다.
   ...
   - stable choice id 후보는 `compare_copied_form_to_old_wound`, `trace_qingliu_eye_variation`, `reconstruct_mumyeongs_sightline`, `stop_before_truth_becomes_accusation`다.
...
- seed 기반 random copy-style system, 천외편린 reward schema, boss combat, route graph/faction reputation/debt/relation ledger, 무명 과거 진실 reveal, `wuxia_qingliu_attack_after_war` full flashback은 열지 않는다.
