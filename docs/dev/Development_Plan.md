...
다음은 `wuxia_boss_recruits_mumyeong_followup` docs-only handoff다. 다음 handoff는 최소 `wuxia_mumyeong_departure_truth_summary`, `wuxia_mumyeong_resolution`, `wuxia_boss_resolution`, `wuxia_seoharin_empty_place`와 기존 late/final 후보를 Notion 사건 카드 DB와 repo hooks로 다시 비교하고, truth delivery/무명 결산/보스 결산/서하린 side beat 중 어느 것을 먼저 runtime으로 승격할지 결정한다.
...
1. 무협 storypack preview/main의 다음 작업은 `wuxia_boss_recruits_mumyeong_followup` docs-only handoff다. `wuxia_boss_recruits_mumyeong` runtime implementation은 완료됐고, 다음 세션은 무명 이탈 진실 정리, 무명 결산, 보스 결산, 서하린의 빈자리 및 기존 late/final 후보를 Notion 사건 카드 DB와 repo hooks로 다시 비교한다.
   ...
   - 보스 첫 등장, 무명의 도움 요청 실패 기록, 무명의 각성, 청류문 흔적 조사, 현악문 멸문 consequence trace, 보스 스카웃 trace는 열었지만 보스 전투/최종 결산, 무명 이탈 진실 정리, 무명 구원 확정은 보류했다. 다음 handoff에서도 truth reveal, 구원 확정, 서하린에게 진실 전달, full flashback, playable 멸문 전투는 바로 열지 않는다.
...
1. 다음 무협 storypack preview/main 작업은 `wuxia_boss_recruits_mumyeong_followup` docs-only handoff다.
   ...
   - stable choice id는 `trace_boss_offer_after_hyeonakmun`, `read_mumyeong_choice_without_excusing_it`, `search_black_serpent_recruitment_record`, `stop_before_following_him_into_black_serpent`다.
   - common hook은 `boss_recruits_mumyeong_resolved`, `boss_recruitment_thread_opened`, `destination_id: cheongryu_outer_courtyard`다.
   - 다음 handoff는 `wuxia_mumyeong_departure_truth_summary`, `wuxia_mumyeong_resolution`, `wuxia_boss_resolution`, `wuxia_seoharin_empty_place`와 기존 late/final 후보를 다시 비교한다.
   - `wuxia_mumyeong_departure_truth_summary`, `wuxia_mumyeong_resolution`, `wuxia_boss_resolution`은 후반 truth/full flashback/final resolution 범위라 이번 runtime slice에서는 열지 않는다.
