...
>
> Rust 전투 개입의 다음 prerequisite 순서는 **I2b-S1a → I2b-S1b → I2b-S2 → I2b-S3**다. S1a는 occurrence target/observer-combatant carrier와 full static roster `CombatOpportunityContext.combatant_ids: BTreeSet<String>`만 소유하고(`combat_opportunity.rs`, wave1 tests, repo 전체 mandatory context literal compile fix), checkpoint/save/lifecycle/transaction/content authoring은 금지한다. Roster는 actual combat participant ID set과 exact-equal하며 active/hp filter가 아니고, evaluation/pause fingerprint에는 stable order로 포함한다. S1b는 `combat_runtime.rs`와 focused tests만 소유해 trigger-tick 시작 health/active snapshot, stable pause ID, selected-target exact-one/no-fallback preflight, checkpoint v2와 provenance-less paused v1 reject를 구현하며 `save.rs`, `lib.rs`, GameState/terminal/transaction/authoring schema는 금지한다. 각 slice는 merge와 독립 review 후 다음 slice로 진행하며 exact acceptance/stop 조건은 [combat contract handoff](../content/design_source/handoffs/combat_contract_handoff.md)의 “WP-I2b-S1 승인 정본”과 “별도 work package와 acceptance”를 따른다. `SaveEnvelope` v2는 I7b까지 금지한다.
>
