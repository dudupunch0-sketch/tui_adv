...
- 런타임 구현: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry`, `wuxia_cheongryu_chore_sparring`, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback` preview는 완료했고, `yageunmong_pack` runtime은 미착수다.
- 다음 추천: `wuxia_baekdo_medicine_debt`를 `righteous_route_started` + `cheongryu_rebuild_thread` 조건으로 preview runtime에 구현한다. direct/deferred 정파 채무 차이는 flavor hook으로만 읽고, 기본 office bundle/`escape-office` key/천외편린 3택 성장/faction route graph schema는 변경하지 않는다.
