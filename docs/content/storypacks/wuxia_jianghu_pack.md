...
- `wuxia_mumyeong_followup_after_failed_aid`에서 도움 요청 실패 기록 이후 이어질 다음 runtime 후보를 Notion event-card DB와 repo runtime hooks로 다시 비교한다.
- 재확인 후보는 최소 `wuxia_mumyeong_departure_truth_summary`, `wuxia_qingliu_attack_after_war`, `wuxia_boss_resolution`, `wuxia_boss_recruits_mumyeong`, `wuxia_mumyeong_awakening`이다.
- 다음 handoff도 truth reveal, full flashback, final boss resolution, reward/ability/combat/route graph schema를 바로 열지 않는 경계를 먼저 확인한다.
