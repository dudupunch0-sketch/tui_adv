...
- `docs/content/storypack_db/encounter_situations.json`: `EncounterSituationCard` 후보 목록. 현재 `isolation_pack` 6개, `yageunmong_pack` 6개, `wuxia_jianghu_pack` 13개, 총 25개 repo 후보 카드를 포함한다. Notion 사건 카드 DB 26개 row는 upstream design source이며, 이 mirror에 자동으로 전부 추가하지 않는다.
...
- `wuxia_heavenly_archive_previous_outsiders`와 `wuxia_wounded_shelter_dawn_offers` preview도 완료했다.
