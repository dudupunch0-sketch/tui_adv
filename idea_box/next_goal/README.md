...
created: 2026-06-01
updated: 2026-06-02
current_goal: wuxia_mumyeong_awakening
mode: implementation
---
...
`wuxia_mumyeong_request_for_aid` runtime implementation은 완료됐다. `wuxia_mumyeong_followup_after_failed_aid` docs-only handoff도 완료됐고, 다음 runtime 후보는 **`wuxia_mumyeong_awakening`**이다. `wuxia_jianghu_pack` / **이구학지 — 천기록**은 Web/terminal default storypack이자 메인 개발 기준이다.

이번 세션의 목표는 **`wuxia_mumyeong_awakening` runtime implementation**이다.

- `src/tui_adv/storypack-previews/wuxia_jianghu_pack/encounters.yaml`에서 `wuxia_mumyeong_request_for_aid` 뒤에 새 encounter를 추가한다.
- Rust/Web storypack preview generated bundle을 갱신한다.
- Python exporter/docs/storypack DB, Rust content bundle, WASM JSON boundary, SuperLightTUI smoke, Web default bundle registry 테스트를 필요한 만큼 갱신한다.
...
- 바로 무명 이탈 진실 전체 reveal, 청류문 습격 full flashback, 보스 최종 결산, 보스 스카웃/정파 문파 멸문, reward schema, combat resolver를 열지 않는다.
...
- `wuxia_boss_first_appearance` stable choice id는 `read_the_boss_flow_and_fail_to_move`, `pull_seo_harin_behind_broken_gate`, `watch_mumyeong_answer_the_boss`, `retreat_before_the_second_step`다.
...
- primary clues:
  ...
  - `protagonist_understands_where_mumyeong_overlays`
...
`wuxia_mumyeong_followup_after_failed_aid`에서 아래 후보를 비교했다.

- `wuxia_mumyeong_awakening`: 도움 요청 실패 기록과 정파 무공 흔적을 무명의 분노/카피 무공 변질로 이을 수 있어 다음 runtime 후보로 선택했다.
