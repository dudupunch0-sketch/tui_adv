...
진행 상태: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry` preview는 완료했다. 다음 구현 slice는 `wuxia_cheongryu_raid_route_split`이며, `wuxia_cheongryu_raid_wounded_fallback`은 raid fallback hook 뒤에만 여는 later candidate다.

- 다섯 preview encounter는 같은 `storypack_preview` bundle에만 둔다.
- 다음 `wuxia_cheongryu_raid_route_split` slice도 기존 encounter schema로 넣는다: `conditions`, `choices`, `cost`, `outcome.resources`, `danger`, `add_flags`, `add_clues`, `add_items`/`remove_items`, `destination_id`, `log`, optional `presentation`.
...
- 상태: 설계 문서화 + machine-readable storypack DB 검증 + preview mode 결정 + 무협 다섯 preview runtime + preview launcher/UI wiring 완료. 다음 `wuxia_cheongryu_raid_route_split`, deferred `wuxia_cheongryu_raid_wounded_fallback` 후보 설계/handoff까지 완료했다.
...
- 런타임 구현: `wuxia_commute_rift_arrival`, `wuxia_heuksa_bang_first_fight`, `wuxia_cheonggi_record_first_fragment`, `wuxia_seo_harin_rescue`, `wuxia_cheongryu_apprentice_entry` preview는 완료했고, `wuxia_cheongryu_raid_route_split`, `wuxia_cheongryu_raid_wounded_fallback`, `yageunmong_pack` runtime은 미착수다.
- 다음 추천: 설계 완료된 `wuxia_cheongryu_raid_route_split`를 `cheongryu_apprentice_entry_resolved`/`cheongryu_trial_started`와 `cheonggi_record_awakened`/`first_fragment_seen` hook으로 이어 붙인다. `wuxia_cheongryu_raid_wounded_fallback`은 raid split fallback branch 뒤 조건부로 열고, 기본 office bundle/`escape-office` key/천외편린 3택 성장/faction route graph schema는 변경하지 않는다.
